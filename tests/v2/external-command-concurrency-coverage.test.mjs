...
const coverage = Object.freeze({
  ...
  'apollo.projects.story-plans.create': {
    mode: 'durable-covered',
    evidence: 'FR-061 actor-bound idempotency, exact immutable ProjectVersion and versioned TreatmentPlan binding, canonical story hash, validation before EditPlan compilation and unique PostgreSQL persistence',
  },
  ...
  'apollo.projects.review-cleanup-masks.create': {
    mode: 'durable-covered', evidence: 'F3.018 actor-bound idempotency and unique content/root revision identities converge duplicate creation while current project, annotation, proxy, source and brief hashes are rechecked',
  },
  'apollo.projects.review-cleanup-masks.refine': {
    mode: 'durable-covered', evidence: 'F3.018 expected mask hash plus latest-root verification and unique root revision persistence fence concurrent append-only refinements without overwriting prior evidence',
  },
  ...
  'apollo.projects.montage-alternatives.create': {
    mode: 'durable-covered', evidence: 'F1.022 actor-bound request fingerprint, immutable content-addressed selection and serializable idempotent persistence',
  },
  ...
  'apollo.projects.color-plan.set': {
    mode: 'durable-covered', evidence: 'serializable exact ProjectVersion, trusted source colorimetry, compiled target set, typed EditCommand and actor-bound replay',
  },
