...
test('F0.031 shell declares eight canonical destinations including localization and retains fail-closed unavailable rendering', () => {
  ...
  assert.equal((appShellRegistrySource.match(/available: true/g) ?? []).length, 8)
...
test('F0.031 workspace selector refreshes identifiers and destroys prior workspace client state', () => {
  ...
  assert.match(workspaceSelectorSource, /startsWith\('apollo-workspace-'\)/)
  assert.match(workspaceSelectorSource, /const router = useRouter\(\)/)
  assert.match(workspaceSelectorSource, /router\.replace\('\/'\)/)
  assert.match(workspaceSelectorSource, /router\.refresh\(\)/)
  assert.doesNotMatch(workspaceSelectorSource, /window\.location\.assign/)
