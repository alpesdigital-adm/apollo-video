...
import { createEditorialAudioTimelineHash, planTalkingHead, TALKING_HEAD_POLICY, TALKING_HEAD_TIMING_GOLDENS, validateProductionCoverage } from '../../src/v2/domain/production-modes.ts'
...
test('T-FR-163 resolves format-specific placement, collisions and impossible constraints with 20 visual goldens', () => { assert.equal(RESPONSIVE_VISUAL_GOLDENS.length,20);const spec=VERSIONED_OUTPUT_PRESETS['9:16'].spec;const result=solveResponsivePlacement({spec,elements:[{id:'a',kind:'insert',anchor:'center',minWidth:.1,maxWidth:.8,minHeight:.1,maxHeight:.8,priority:2,readingOrder:0},{id:'impossible',kind:'cta',anchor:'center',minWidth:.95,maxWidth:.95,minHeight:.95,maxHeight:.95,priority:1,readingOrder:1}]});assert.equal(result.elements[0].id,'a');assert.equal(result.issues.at(-1).code,'IMPOSSIBLE_CONSTRAINTS') })
