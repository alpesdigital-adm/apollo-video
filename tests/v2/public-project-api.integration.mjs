...
test('authenticated public API manages projects, clients and artifact inspection', async () => {
  ...
  const cleanup = async () => {
    ...
    await client.v2GovernancePolicyCommand.deleteMany({ where: { workspaceId: { in: workspaceIds } } })
  ...
  try {
    ...
    assert.equal(recoveredReplayResponse.status, 200, JSON.stringify(recoveredReplay))
