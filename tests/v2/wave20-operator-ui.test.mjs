...
test('W20-UI the descoped F4.016 gate page stays a decision with an expiry', () => {
  // The brief names four operator surfaces and three were built. The fourth
  // cannot be: no gate capability is published, so there is no /v1 path for a
  // page to read, and a page that reached the service any other way would break
  // the rule the whole wave is about.
  //
  // This is the part that keeps the note from rotting into an excuse: the day
  // the API lane publishes the gate, the reason written down in
  // docs/quality/wave20-operator-surfaces.md stops being true and this fails,
  // which is the only kind of descope record worth having.
  const note = read('docs/quality/wave20-operator-surfaces.md')
  assert.match(note, /F4\.016/, 'the descope note no longer names the surface it is about')

  const gateCapabilities = FOUNDATION_CAPABILITIES
    .map((capability) => capability.id)
    .filter((id) => /gate/i.test(id) && !id.includes('mvp-core-gates'))
  assert.deepEqual(
    gateCapabilities, [],
    'the F4.016 gate is published now, so the operator page the brief asks for can and must be built; '
    + 'rewrite docs/quality/wave20-operator-surfaces.md before deleting this assertion',
  )
})
