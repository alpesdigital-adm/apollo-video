...
test('T-FR-163 public request crosses the exact parser and canonical application service', async () => {
  const request = structuredClone(publicSchemaExamples(getPublicSchema('apollo://schemas/responsive-placement-request/v1'))[0])
  const result = await solveResponsivePlacementService()(parseResponsivePlacementBody(request))

  assert.equal(result.schemaVersion, 'responsive-placement/v1')
...
test('T-FR-163 public capability is authenticated, naturally idempotent and V2-only', async () => {
  ...
  assert.deepEqual(capability, {
    id: 'apollo.responsive-placement.solve', version: '1.0.0', title: 'Solve responsive placement',
    description: capability.description, exposure: 'public', operationKind: 'preflight', authMode: 'required',
    requiredScopes: ['projects:read'], inputSchemaRef: 'apollo://schemas/responsive-placement-request/v1',
    outputSchemaRef: 'apollo://schemas/responsive-placement-result/v1', endpoint: { method: 'POST', path: '/v1/responsive-placement/solve' },
