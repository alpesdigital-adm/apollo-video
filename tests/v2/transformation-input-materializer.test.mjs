...
const SOURCE_PATH = resolve('package.json')
...
function job(sourceHash = SOURCE_HASH) {
  return createProviderJob({
    ...
    providerInput: { schemaVersion: 'transformation-provider-input/v1', sourceArtifactHash: sourceHash, cleanupMask: { maskId: 'mask-transformation-reviewed', maskHash: HASH('4') } },
...
function materializer(sha256 = SOURCE_HASH) {
  let cleaned = 0
  return new AuthorizedProviderSubmissionInputMaterializer({
    profiles: {},
    sources: {
      async materialize() { return { path: SOURCE_PATH, sha256: SOURCE_HASH, byteSize: SOURCE_BYTES.byteLength } },
      async cleanup() { cleaned += 1 },
    },
    clock: () => new Date('2026-09-01T21:30:00.000Z'),
    artifacts: { async findById(workspaceId, artifactId) { return { id: artifactId, workspaceId, artifactKey: 'test/package.json', sha256, byteSize: BigInt(SOURCE_BYTES.byteLength), status: 'available', mediaType: 'video', container: 'mp4' } } },
  })
}
...
test('T-FR-218 transformation materializer revalidates source and preserves sealed mask projection', async () => {
  ...
  assert.notDeepEqual(input, source.input)
  assert.equal(input.cleanupMask.maskHash, HASH('4'))
  assert.equal(input.sourceMediaSha256, SOURCE_HASH)
  assert.equal(input.sourceMediaByteSize, SOURCE_BYTES.byteLength)
  assert.deepEqual(Buffer.from(input.sourceMediaBase64, 'base64'), SOURCE_BYTES)
})
