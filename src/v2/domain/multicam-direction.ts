 ...
 * gates it by role only (`domain/capture-session.ts:383-387`);
 * `syncAudioPolicy` is what the audio IS — `'none'` carries no sync and no
 * final content, `'sync-only'` lines clocks up
 * (`domain/capture-session.ts:114-119`). `'none' + includeInFinalMix` is a legal
