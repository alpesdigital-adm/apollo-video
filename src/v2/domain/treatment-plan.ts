...
export interface TreatmentPlan {
  schemaVersion: 2; objective: StrategicObjectiveId; mode: ProductionMode; confidence: number; energy: number; visualDensity: number
...
export function createTreatmentPlan(input: { objective: StrategicObjectiveId; mode: ProductionMode; rubric: { id: string; version: number; proofRequired: boolean }; policy: { snapshotId: string; maxPatternBreaksPer30s: number; forbiddenEffects: readonly string[] }; perception: { summaryId: string; confidence: number; speakerCoverage: number; visualVariety: number }; mediaOnly?: { confidence: number; assumptions: readonly string[]; observedClaims: readonly string[]; proposedClaims: readonly string[] } }): Readonly<TreatmentPlan> {
  ...
  const plan: TreatmentPlan = { schemaVersion: 2, objective: input.objective, mode: input.mode, confidence: input.mediaOnly?.confidence ?? input.perception.confidence, energy, visualDensity, grammar: { primary: input.mode !== 'visual-montage' && input.perception.speakerCoverage >= .5 ? 'speaker' : 'b-roll', shotRhythm: energy >= .75 ? 'dynamic' : 'measured', subtitleMode: input.mode === 'visual-montage' ? 'narrative' : 'support' }, patternBreaks: { maxPer30s: Math.min(input.policy.maxPatternBreaksPer30s, energy >= .75 ? 5 : 3), allowed: Object.freeze(allowed) }, proofPolicy: { required: input.rubric.proofRequired || conversion, minimumEvidenceItems: input.rubric.proofRequired || conversion ? 1 : 0 }, ctaPolicy: { required: conversion, placement: conversion ? 'close' : 'none' }, assumptions: Object.freeze([...assumptions]), alternatives: Object.freeze([{ id: 'lower-density', difference: 'Reduce visual density by 20% while preserving narrative structure.' }]), decisions: Object.freeze([{ field: 'energy', evidenceRefs: Object.freeze([input.rubric.id]), reason: `Objective ${input.objective} calibrated energy.` }, { field: 'grammar.primary', evidenceRefs: Object.freeze([input.perception.summaryId]), reason: 'Primary visual follows observed speaker coverage and production mode.' }]), claimPolicy: Object.freeze({ observedClaims: Object.freeze([...(input.mediaOnly?.observedClaims ?? [])]), proposedClaims: Object.freeze([...(input.mediaOnly?.proposedClaims ?? [])]) }), provenance: Object.freeze({ rubricId: input.rubric.id, rubricVersion: input.rubric.version, policySnapshotId: input.policy.snapshotId, perceptionSummaryId: input.perception.summaryId }) }
  return validateTreatmentPlan(plan)
}

export function validateTreatmentPlan(plan: TreatmentPlan): Readonly<TreatmentPlan> {
  if (plan.schemaVersion !== 2 || !Number.isFinite(plan.confidence) || plan.confidence < 0 || plan.confidence > 1) throw new DomainError('INVALID_ARGUMENT', 'Treatment confidence or schema version is invalid')
  if (!Array.isArray(plan.assumptions) || !plan.claimPolicy || !Array.isArray(plan.claimPolicy.observedClaims) || !Array.isArray(plan.claimPolicy.proposedClaims)) throw new DomainError('INVALID_ARGUMENT', 'Treatment assumptions and claim policy are invalid')
