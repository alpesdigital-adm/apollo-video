...
export interface ValidationEnvelope {
  segmentId: string
  protected: readonly (
    'copy' | 'take' | 'framing' | 'timing' | 'opening'
  )[]
...
export function applyValidationEnvelope(
  ...
  const violations = input.changes.filter((change) =>
    envelope.protected.includes(change))
  return Object.freeze({
