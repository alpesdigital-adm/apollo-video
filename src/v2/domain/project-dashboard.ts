...
export function deriveDashboardProject(input: { status: string; completed?: number | null; total?: number | null; archivedAt?: string | null }) {
  const state: DashboardProjectState = input.archivedAt || input.status === 'archived' ? 'archived' : input.status === 'error' ? 'failed' : input.status === 'complete' ? 'completed' : input.status === 'ready' ? 'awaiting-review' : ['uploading', 'normalizing', 'transcribing', 'analyzing', 'rendering'].includes(input.status) ? 'processing' : 'draft'
