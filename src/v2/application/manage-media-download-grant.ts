...
export function issueMediaDownloadGrantService(dependencies: {
  ...
  return async function issue(input: { workspaceId: string; actor: AuthenticatedExternalActor; artifactId: string; idempotencyKey: string; ttlSeconds?: number }) {
    ...
    const requestFingerprint = calculateVersionHash({
      schemaVersion: 'media-download-grant-issue/v2',
      artifactId: artifact!.id,
      ttlSeconds,
      actorContextHash: audit.contextHash,
    })
    const signed = dependencies.signer.sign({ grantId: id, workspaceId: input.workspaceId, clientId: audit.clientId, artifactId: artifact!.id, expiresAt })
    const candidate = createMediaDownloadGrant({ id, workspaceId: input.workspaceId, clientId: audit.clientId, artifactId: artifact!.id, tokenHash: hashToken(signed.token), idempotencyKey: input.idempotencyKey, requestFingerprint, status: 'active', createdAt: createdAt.toISOString(), expiresAt, audit })
...
export function revokeMediaDownloadGrantService(dependencies: { grants: MediaDownloadGrantRepository; clock?: () => Date }) {
  const clock = dependencies.clock ?? (() => new Date())
  return async function revoke(input: { workspaceId: string; actor: AuthenticatedExternalActor; grantId: string }) {
    requireScope(input.actor, 'artifacts:read')
    if (input.actor.workspaceId !== input.workspaceId) {
      throw new DomainError('MEDIA_DOWNLOAD_GRANT_NOT_FOUND', 'Media download grant was not found')
    }
    const audit = materializeActorAuditContext(input.actor)
    return dependencies.grants.revokeOrReplay({
