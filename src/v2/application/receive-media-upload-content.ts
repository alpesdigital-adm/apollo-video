...
import { recordMediaUploadPartFromSessionService } from './manage-media-upload.ts'
...
export function receiveMediaUploadContentService(dependencies: {
  ...
  return async function receive(input: {
    ...
    assertDomain(upload!.sessionExpiresAt === input.sessionExpiresAt, 'MEDIA_UPLOAD_TRANSITION_REJECTED', 'Signed upload session was replaced')
    ...
    if (input.mode === 'multipart') {
      const sessionAudit = await dependencies.repository.findCurrentUploadSessionAudit({
        workspaceId: input.workspaceId, clientId: input.clientId,
        uploadId: input.uploadId, sessionExpiresAt: input.sessionExpiresAt,
      })
      assertDomain(Boolean(sessionAudit), 'PERSISTENCE_CONFLICT', 'Signed upload session audit is missing')
      const part = await recordMediaUploadPartFromSessionService({ repository: dependencies.repository, clock })({
