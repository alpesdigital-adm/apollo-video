...
export class TesseractImageVisionProvider implements ImageVisionProvider {
  ...
  async analyze(input: { sourcePath: string; width: number; height: number; signal?: AbortSignal }) {
    ...
    const inferredTags = Object.freeze(values.flatMap((region) => region.text.toLocaleLowerCase().split(/[^\p{L}\p{N}]+/u).filter((word) => word.length >= 4).map((value) => Object.freeze({ value, confidence: region.confidence, provenance: `tesseract:${this.languages.join('-')}@v5:ocr:${region.language}` }))))
