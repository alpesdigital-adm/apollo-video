...
export interface StoredExternalActorAudit {
  workspaceId: string
...
function canonicalAudit(
  row: Readonly<StoredExternalActorAudit>,
  ...
  if (
    ...
    throw new DomainError(
      ...
      'Stored external mutation predates credential-bound audit',
    )
  }
  try {
    const audit = createApiAccessAuditContext({
      ...
      environment:
        row.actorEnvironment as ApiAccessAuditContext['environment'],
      authenticationKind:
      ...
      ...(row.workspaceRole
        ? {
            workspaceRole:
              row.workspaceRole as ApiAccessAuditContext['workspaceRole'],
          }
        : {}),
    })
    if (audit.contextHash !== row.actorContextHash) {
      throw new DomainError(
        'PERSISTENCE_CONFLICT',
        'Stored external mutation audit hash is inconsistent',
      )
    }
    return audit
  } catch (error) {
    ...
    throw new DomainError(
      'PERSISTENCE_CONFLICT',
      'Stored external mutation audit context is invalid',
    )
  }
}

export function hydrateExternalActorAudit(
  row: Readonly<StoredExternalActorAudit>,
  actorClientId: string,
): Readonly<ApiAccessAuditContext> {
...
export function externalActorAuditData(
  ...
  if (
    ...
    throw new DomainError(
      'AUTH_INVALID',
      'External mutation audit does not match its workspace and actor',
    )
  }
  return {
