...
const storyPlanCorePropertiesV2 = {
  productionMode: { const: 'hybrid' },
  objective: strategicObjectiveSchema,
  ...
  blocks: { type: 'array', minItems: 1, items: { type: 'object', additionalProperties: false, required: ['id', 'actId', 'role', 'intent', 'dependencies', 'sourceCandidateIds', 'durationTargetMs', 'content', 'presentation'], properties: { id: idSchema, actId: idSchema, role: { enum: ['hook', 'context', 'argument', 'proof', 'cta'] }, intent: { type: 'string', minLength: 1, maxLength: 512 }, dependencies: { type: 'array', items: idSchema }, sourceCandidateIds: { type: 'array', minItems: 1, items: idSchema }, durationTargetMs: { type: 'object', additionalProperties: false, required: ['min', 'ideal', 'max'], properties: { min: { type: 'integer', minimum: 1 }, ideal: { type: 'integer', minimum: 1 }, max: { type: 'integer', minimum: 1 } } }, content: { type: 'object', additionalProperties: false, required: ['claimIds', 'qualifierIds', 'proofIds'], properties: { claimIds: { type: 'array', items: idSchema }, qualifierIds: { type: 'array', items: idSchema }, proofIds: { type: 'array', items: idSchema }, ctaId: idSchema } }, presentation: { enum: ['source-video', 'voiceover', 'cold-open-reference', 'synthetic-avatar', 'proof-insert', 'b-roll'] }, sourceRangeId: idSchema } } },
  sourceRanges: { type: 'array', minItems: 1, items: { type: 'object', additionalProperties: false, required: ['id', 'artifactId', 'startMs', 'endMs', 'rightsRef'], properties: { id: idSchema, artifactId: idSchema, startMs: { type: 'integer', minimum: 0 }, endMs: { type: 'integer', minimum: 1 }, rightsRef: idSchema, consentRef: idSchema, sourceKind: { enum: ['real', 'synthetic', 'voiceover', 'proof', 'b-roll'] }, identityRef: idSchema, audioContinuityRef: idSchema, sceneContinuityRef: idSchema, disclosure: { type: 'string', minLength: 1, maxLength: 500 } } } },
...
const { productionMode: _storyProductionMode, blocks: _storyBlocksV2, sourceRanges: _storyRangesV2, ...storyPlanSharedProperties } = storyPlanCorePropertiesV2
const storyPlanCorePropertiesV1 = {
  ...storyPlanSharedProperties,
...
const storyPlanEnvelopeProperties = { id: idSchema, workspaceId: idSchema, projectId: idSchema, projectVersionId: idSchema, storyHash: sha256Schema, createdBy: { type: 'object', additionalProperties: false, required: ['type', 'id'], properties: { type: { const: 'api-client' }, id: idSchema } }, createdAt: dateTimeSchema, requestFingerprint: sha256Schema } as const
const storyPlanSchemaV1 = { type: 'object', additionalProperties: false, required: ['schemaVersion', 'id', 'workspaceId', 'projectId', 'projectVersionId', 'objective', 'desiredActionRef', 'treatmentPlanRef', 'targetDurationMs', 'acts', 'blocks', 'sourceRanges', 'sourceCandidates', 'qualifiers', 'claims', 'proofContexts', 'storyHash', 'createdBy', 'createdAt', 'requestFingerprint'], properties: { schemaVersion: { const: 3 }, id: idSchema, workspaceId: idSchema, projectId: idSchema, projectVersionId: idSchema, ...storyPlanCorePropertiesV1, storyHash: sha256Schema, createdBy: storyPlanEnvelopeProperties.createdBy, createdAt: dateTimeSchema, requestFingerprint: sha256Schema } } as const
const storyPlanSchemaV2 = { type: 'object', additionalProperties: false, required: ['schemaVersion', 'id', 'workspaceId', 'projectId', 'projectVersionId', 'objective', 'desiredActionRef', 'treatmentPlanRef', 'targetDurationMs', 'acts', 'blocks', 'sourceRanges', 'sourceCandidates', 'qualifiers', 'claims', 'proofContexts', 'storyHash', 'createdBy', 'createdAt', 'requestFingerprint'], properties: { schemaVersion: { enum: [3, 4] }, id: idSchema, workspaceId: idSchema, projectId: idSchema, projectVersionId: idSchema, ...storyPlanCorePropertiesV2, storyHash: sha256Schema, createdBy: storyPlanEnvelopeProperties.createdBy, createdAt: dateTimeSchema, requestFingerprint: sha256Schema }, allOf: [{ if: { properties: { schemaVersion: { const: 4 } } }, then: { required: ['productionMode'], properties: { productionMode: { const: 'hybrid' } } } }] } as const
...
const syntheticRangeSchema: JsonSchema = {
  type: 'array', minItems: 2, maxItems: 2,
  items: { type: 'integer', minimum: 0 },
}
...
const syntheticPresenterProfileSchema: JsonSchema = {
  type: 'object', additionalProperties: false,
  required: [
    'id', 'version', 'snapshotHash', 'actorIdentityId', 'avatar', 'voice',
    'defaultLocale', 'status', 'disclosure', 'consent', 'createdAt',
  ],
  properties: {
    id: idSchema, version: { type: 'integer', minimum: 1 }, snapshotHash: sha256Schema,
    actorIdentityId: idSchema,
    avatar: {
      type: 'object', additionalProperties: false,
      required: ['adapterId', 'adapterVersion', 'identityRef'],
      properties: { adapterId: idSchema, adapterVersion: idSchema, identityRef: idSchema },
    },
    voice: {
      type: 'object', additionalProperties: false,
      required: ['id', 'version', 'adapterId', 'adapterVersion'],
      properties: { id: idSchema, version: { type: 'integer', minimum: 1 }, adapterId: idSchema, adapterVersion: idSchema },
    },
    defaultLocale: { type: 'string', minLength: 2, maxLength: 35 },
    status: { enum: ['active', 'disabled', 'expired'] }, disclosure: { type: 'string', minLength: 3, maxLength: 500 },
    consent: {
      type: 'object', additionalProperties: false,
      required: ['id', 'evidenceArtifactId', 'evidenceSha256', 'snapshotHash', 'granted', 'allowedUses', 'allowedMarkets', 'allowedLocales', 'allowedOperations', 'expiresAt'],
      properties: {
        id: idSchema, evidenceArtifactId: idSchema, evidenceSha256: sha256Schema, snapshotHash: sha256Schema,
...
const syntheticProductionPlanSchema: JsonSchema = {
  type: 'object', additionalProperties: false,
  required: [
    'schemaVersion', 'policyVersion', 'id', 'workspaceId', 'projectId',
    'projectVersionId', 'mode', 'hasRealPerson', 'durationMs', 'use', 'market',
    'locale', 'profile', 'audio', 'blocks', 'bRoll', 'overlays', 'captions',
    'disclosure', 'authorization', 'createdAt', 'planHash',
  ],
  properties: {
    schemaVersion: { const: 'synthetic-edit-plan/v1' },
    policyVersion: { const: 'synthetic-presenter-policy/v1' },
    id: idSchema, workspaceId: idSchema, projectId: idSchema, projectVersionId: idSchema,
    mode: { const: 'synthetic-presenter' }, hasRealPerson: { const: false },
    durationMs: { type: 'integer', minimum: 1 }, use: idSchema, market: { type: 'string' }, locale: { type: 'string' },
    profile: { type: 'object' },
    audio: { type: 'object' }, blocks: { type: 'array', minItems: 1, maxItems: 500, items: { type: 'object' } },
    bRoll: { type: 'array', maxItems: 500, items: { type: 'object' } }, overlays: { type: 'array', maxItems: 500, items: { type: 'object' } },
    captions: { type: 'array', maxItems: 100000, items: { type: 'object' } }, disclosure: { type: 'string', minLength: 3, maxLength: 500 },
...
// ---------------------------------------------------------------------------
// Wave 18 — shared shapes for capture sessions and multi-range synthesis.
//
// Ticks and rates cross the public boundary as strings. A JSON number is an
// IEEE 754 double in every mainstream parser, so a 64-bit tick would arrive at
// a client already rounded, with nothing raised and no way to notice; and
// 30000/1001 has no decimal form at all, so a client handed 29.97 could never
// recover the rate it was sent.
// ---------------------------------------------------------------------------

const tickSchema = { type: 'string', pattern: '^-?[0-9]{1,19}$' }
const unsignedTickSchema = { type: 'string', pattern: '^[0-9]{1,19}$' }
const rationalSchema = { type: 'string', pattern: '^[1-9][0-9]{0,18}/[1-9][0-9]{0,18}$' }

/** Half-open, [start, end). Adjacent intervals tile with no gap and no overlap. */
const tickIntervalSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['start', 'end'],
  properties: { start: tickSchema, end: tickSchema },
}

const captureClockPolicySchema = {
  type: 'object',
  additionalProperties: false,
  required: ['timebase', 'rounding'],
  properties: { timebase: rationalSchema, rounding: { enum: ['nearest-half-even', 'floor', 'ceil'] } },
}

const captureCommandLineageSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['commandId', 'actorKind', 'actorId'],
  properties: {
    commandId: idSchema,
    actorKind: { enum: ['human', 'api-client', 'director'] },
    actorId: idSchema,
    note: { type: ['string', 'null'], maxLength: 1024 },
  },
}

const captureDeviceSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['deviceId', 'recorderId'],
  properties: {
    deviceId: idSchema,
    recorderId: idSchema,
    make: { type: ['string', 'null'], maxLength: 128 },
    model: { type: ['string', 'null'], maxLength: 128 },
    serial: { type: ['string', 'null'], maxLength: 128 },
  },
}

const captureTrackPartInputSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['partId', 'ordinal', 'sourceAssetId', 'timebase', 'coverage', 'streamIndex',
    'splitReason', 'evidence'],
  properties: {
    partId: idSchema,
    ordinal: { type: 'integer', minimum: 0, maximum: 100_000 },
    sourceAssetId: idSchema,
    // The part's OWN timebase, exactly as the recorder wrote it. Never the
    // transcode's: a normalized file is an output of the pipeline, and letting
    // it define time means every timestamp moves when a setting changes.
    timebase: rationalSchema,
    coverage: tickIntervalSchema,
    streamIndex: { type: 'integer', minimum: 0, maximum: 1_024 },
    splitReason: {
      enum: ['single-file', 'recorder-restart', 'file-size-limit', 'card-change',
        'clip-duration-limit', 'unknown'],
    },
    evidence: {
      type: 'object',
      additionalProperties: false,
      required: ['ingestArtifactId', 'ingestSha256', 'probeHash', 'probeSource', 'observedAt'],
      properties: {
...
const captureTrackInputSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['trackId', 'role', 'device', 'timebase', 'streamIndex', 'syncAudioPolicy',
    'includeInFinalMix', 'firstPart'],
  properties: {
    trackId: idSchema,
    role: {
      enum: ['camera-main', 'camera-alt', 'screen', 'phone', 'reaction',
        'reference-video', 'microphone', 'master-audio', 'scratch-audio'],
    },
    device: captureDeviceSchema,
    timebase: rationalSchema,
    streamIndex: { type: 'integer', minimum: 0, maximum: 1_024 },
    syncAudioPolicy: { enum: ['available', 'none', 'sync-only', 'final-candidate'] },
    includeInFinalMix: { type: 'boolean' },
    firstPart: captureTrackPartInputSchema,
  },
}

const captureTrackSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['trackId', 'role', 'device', 'sourceAssetId', 'timebase', 'streamIndex',
    'syncAudioPolicy', 'includeInFinalMix', 'parts'],
  properties: {
    trackId: idSchema,
    role: {
      enum: ['camera-main', 'camera-alt', 'screen', 'phone', 'reaction',
    ...
    syncAudioPolicy: { enum: ['available', 'none', 'sync-only', 'final-candidate'] },
    includeInFinalMix: { type: 'boolean' },
    parts: { type: 'array', items: captureTrackPartInputSchema },
  },
}

const captureSessionSummarySchema = {
  type: 'object',
  additionalProperties: false,
  required: ['sessionId', 'version', 'status', 'sessionHash', 'referenceTrackId',
    'referenceEpoch', 'trackCount'],
  properties: {
    sessionId: idSchema,
    version: { type: 'integer', minimum: 1 },
    previousVersionHash: { type: ['string', 'null'], pattern: '^[a-f0-9]{64}$' },
    status: { enum: ['draft', 'analyzing', 'needs-input', 'synced', 'partial', 'failed'] },
    sessionHash: sha256Schema,
    referenceTrackId: idSchema,
    referenceEpoch: { type: 'integer', minimum: 1 },
    ...
    staleDerivations: {
      type: 'array',
      items: { enum: ['track-coverage', 'session-clock-map', 'sync-diagnostic', 'edit-plan'] },
    },
  },
}
...
const captureSessionSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['sessionId', 'projectId', 'version', 'status', 'clock', 'referenceTrackId',
    'referenceEpoch', 'tracks', 'lineage', 'sessionHash', 'createdAt'],
  properties: {
    sessionId: idSchema,
    projectId: idSchema,
    version: { type: 'integer', minimum: 1 },
    previousVersionHash: { type: ['string', 'null'], pattern: '^[a-f0-9]{64}$' },
    status: { enum: ['draft', 'analyzing', 'needs-input', 'synced', 'partial', 'failed'] },
    clock: captureClockPolicySchema,
    referenceTrackId: idSchema,
    referenceEpoch: { type: 'integer', minimum: 1 },
    tracks: { type: 'array', items: captureTrackSchema },
    lineage: {
      type: 'object',
      additionalProperties: false,
      required: ['commandId', 'operation', 'actorKind', 'actorId', 'occurredAt'],
      properties: {
        commandId: idSchema,
        operation: {
          enum: ['create-session', 'add-track', 'add-track-part',
...
const clockMapPieceSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['pieceId', 'ordinal', 'sourceCoverage', 'sessionCoverage', 'rate', 'offsetTicks',
    'driftPpm', 'confidence', 'residualBoundTicks'],
  properties: {
    pieceId: idSchema,
    ordinal: { type: 'integer', minimum: 0 },
    sourceCoverage: tickIntervalSchema,
    sessionCoverage: tickIntervalSchema,
    rate: rationalSchema,
    offsetTicks: tickSchema,
    driftPpm: { type: 'integer' },
    confidence: { enum: ['high', 'medium', 'low'] },
    residualBoundTicks: unsignedTickSchema,
    // Null only for the first piece: nothing precedes it to be a boundary with.
    openedBy: {
      type: ['string', 'null'],
      enum: ['recorder-restart', 'pts-regression', 'seek', 'rewind', 'file-split',
        'coverage-gap', 'residual-exceeded', 'manual-anchor-conflict', null],
    },
    openedByDetail: { type: ['string', 'null'], maxLength: 512 },
    anchorIds: { type: 'array', items: idSchema },
    evidenceRefs: { type: 'array', items: { type: 'string', maxLength: 512 } },
  },
}

const captureSyncTrackSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['trackId', 'outcome'],
  properties: {
    trackId: idSchema,
    // `insufficient-evidence` is a first-class answer, not a failure. It is the
    // difference between "we could not tell" and "we measured zero".
    outcome: { enum: ['auto-apply', 'review', 'insufficient-evidence'] },
    manualRequired: { type: 'boolean' },
    selectedMethod: {
      type: ['string', 'null'],
      enum: ['shared-timecode', 'trusted-metadata', 'apollo-marker', 'audio-fingerprint',
        'visual-event', 'transcript-lip', 'manual-anchor', null],
    },
    outcomeReasons: { type: 'array', items: { type: 'string', maxLength: 512 } },
    // Absent whenever the cascade could not tell. Never an offset of zero, and
    // never interpolated across the gap between two pieces.
    map: {
      type: ['object', 'null'],
      additionalProperties: false,
      required: ['sourceBounds', 'pieces'],
      properties: {
        sourceBounds: tickIntervalSchema,
        uncovered: { type: 'array', items: tickIntervalSchema },
        pieces: { type: 'array', items: clockMapPieceSchema },
      },
    },
    coverage: {
      type: ['object', 'null'],
      additionalProperties: false,
      required: ['bounds', 'coveredTicks', 'gapTicks', 'autoEditable'],
      properties: {
        bounds: tickIntervalSchema,
        coveredTicks: unsignedTickSchema,
        gapTicks: unsignedTickSchema,
        minConfidenceBps: { type: 'integer', minimum: 0, maximum: 10_000 },
        autoEditable: { type: 'boolean' },
        unresolvedOverlaps: { type: 'integer', minimum: 0 },
      },
    },
  },
}

const synthesisLineageSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['sourceArtifactId', 'sourceArtifactSha256', 'sourceManifestId', 'sourceManifestHash',
    'indexRunId', 'momentId', 'momentHash', 'evaluationId', 'evaluationHash'],
  properties: {
    sourceArtifactId: idSchema,
    sourceArtifactSha256: sha256Schema,
    sourceManifestId: idSchema,
...
const synthesisRangeInputSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['rangeId', 'startMs', 'endMs', 'lineage', 'rightsSnapshotId', 'rightsStatus',
    'consentStatus'],
  properties: {
    rangeId: idSchema,
    startMs: { type: 'integer', minimum: 0 },
    endMs: { type: 'integer', minimum: 1 },
    lineage: synthesisLineageSchema,
    rightsSnapshotId: idSchema,
    // Re-checked when the cut is assembled rather than trusted from selection:
    // a window can be chosen while approved and assembled after a revocation.
    rightsStatus: { enum: ['approved', 'blocked'] },
    consentStatus: { enum: ['approved', 'not-required', 'blocked'] },
    claimIds: { type: 'array', items: idSchema },
    qualifierIds: { type: 'array', items: idSchema },
    proofContextIds: { type: 'array', items: idSchema },
  },
}

const synthesisJoinInputSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['beforeRangeId', 'afterRangeId', 'kind', 'justification'],
  properties: {
    beforeRangeId: idSchema,
    afterRangeId: idSchema,
    // Checked against the source, never taken on trust: labelling a splice
    // "contiguous" claims the speaker said those words consecutively.
    kind: { enum: ['contiguous', 'spliced'] },
    justification: { type: 'string', maxLength: 1_024 },
    continuityRisks: {
      type: 'array',
      items: { enum: ['argument', 'audio', 'eye-line', 'position', 'color'] },
    },
  },
}

const editorialSynthesisSummarySchema = {
  type: 'object',
  additionalProperties: false,
  required: ['synthesisId', 'objective', 'synthesizedDurationMs', 'sourceDurationMs',
    'rangeCount', 'spliceCount', 'chronologyPreserved', 'synthesisHash'],
  properties: {
    synthesisId: idSchema,
    objective: { type: 'string', maxLength: 512 },
    synthesizedDurationMs: { type: 'integer', minimum: 1 },
    sourceDurationMs: { type: 'integer', minimum: 1 },
    droppedMs: { type: 'integer', minimum: 0 },
    compressionBps: { type: 'integer', minimum: 0, maximum: 10_000 },
    rangeCount: { type: 'integer', minimum: 1 },
    spliceCount: { type: 'integer', minimum: 0 },
    chronologyPreserved: { type: 'boolean' },
    reorderReason: { type: ['string', 'null'], maxLength: 512 },
    durationFrames: { type: 'integer', minimum: 1 },
    editPlanSelectionHash: sha256Schema,
    synthesisHash: sha256Schema,
  },
}

const editorialSynthesisSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['synthesisId', 'projectId', 'objective', 'targetDurationMs', 'toleranceMs',
    'synthesizedDurationMs', 'sourceDurationMs', 'droppedMs', 'chronologyPreserved',
    'storyPlanId', 'editPlanId', 'frameRate', 'ranges', 'joins', 'contextProof',
    'synthesisHash', 'createdAt'],
  properties: {
    synthesisId: idSchema,
    projectId: idSchema,
    objective: { type: 'string', maxLength: 512 },
    targetDurationMs: { type: 'integer', minimum: 1 },
    ...
    ranges: {
      ...
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['rangeId', 'startMs', 'endMs', 'lineage', 'rightsSnapshotId',
          'rightsStatus', 'consentStatus'],
        properties: synthesisRangeInputSchema.properties,
      },
    },
    joins: {
      type: 'array',
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['beforeRangeId', 'afterRangeId', 'kind', 'droppedMs', 'timelineMs',
          'justification'],
        properties: {
          ...synthesisJoinInputSchema.properties,
          droppedMs: { type: 'integer', minimum: 0 },
          timelineMs: { type: 'integer', minimum: 0 },
        },
      },
    },
    // What the context check examined, kept whether or not it found anything.
    // A proof that only exists on failure cannot be told apart, afterwards,
    // from a branch that never ran.
    contextProof: {
      type: 'object',
      additionalProperties: false,
      required: ['claimsIncluded', 'qualifiersIncluded', 'proofContextsIncluded',
        'claimsRequiringQualifiers', 'claimsRequiringProof'],
      properties: {
        claimsIncluded: { type: 'array', items: idSchema },
        qualifiersIncluded: { type: 'array', items: idSchema },
        proofContextsIncluded: { type: 'array', items: idSchema },
        claimsRequiringQualifiers: { type: 'integer', minimum: 0 },
        claimsRequiringProof: { type: 'integer', minimum: 0 },
      },
    },
    lineageRefs: { type: 'array', items: { type: 'string', maxLength: 256 } },
    synthesisHash: sha256Schema,
    createdAt: { type: 'string', format: 'date-time' },
  },
}
...
export const PUBLIC_SCHEMAS = defineSchemaRegistry([
  ...
  defineSchema('create-story-plan-request', 1, 'Create a validated StoryPlan before EditPlan compilation', { type: 'object', additionalProperties: false, required: ['projectVersionId', 'plan'], properties: { projectVersionId: idSchema, plan: { type: 'object', additionalProperties: false, required: ['objective', 'desiredActionRef', 'treatmentPlanRef', 'targetDurationMs', 'acts', 'blocks', 'sourceRanges', 'sourceCandidates', 'qualifiers', 'claims', 'proofContexts'], properties: storyPlanCorePropertiesV1 } } }),
  defineSchema('story-plan-mutated', 1, 'Created or replayed immutable StoryPlan', successSchema({ type: 'object', additionalProperties: false, required: ['storyPlan', 'replayed'], properties: { storyPlan: storyPlanSchemaV1, replayed: { type: 'boolean' } } })),
  defineSchema('story-plan-read', 1, 'Immutable validated StoryPlan', successSchema({ type: 'object', additionalProperties: false, required: ['storyPlan'], properties: { storyPlan: storyPlanSchemaV1 } })),
  defineSchema('create-story-plan-request', 2, 'Create a validated source or hybrid StoryPlan before EditPlan compilation', { type: 'object', additionalProperties: false, required: ['projectVersionId', 'plan'], properties: { projectVersionId: idSchema, plan: { type: 'object', additionalProperties: false, required: ['objective', 'desiredActionRef', 'treatmentPlanRef', 'targetDurationMs', 'acts', 'blocks', 'sourceRanges', 'sourceCandidates', 'qualifiers', 'claims', 'proofContexts'], properties: storyPlanCorePropertiesV2 } } }),
  defineSchema('story-plan-mutated', 2, 'Created or replayed immutable source or hybrid StoryPlan', successSchema({ type: 'object', additionalProperties: false, required: ['storyPlan', 'replayed'], properties: { storyPlan: storyPlanSchemaV2, replayed: { type: 'boolean' } } })),
  defineSchema('story-plan-read', 2, 'Immutable validated source or hybrid StoryPlan', successSchema({ type: 'object', additionalProperties: false, required: ['storyPlan'], properties: { storyPlan: storyPlanSchemaV2 } })),
  ...
  defineSchema('sandbox-provider-execution-page', 2, 'Redacted simulated provider execution receipts across semantic and media providers', successSchema({
    type: 'object', additionalProperties: false, required: ['entries'], properties: {
      entries: {
        type: 'array', maxItems: 100, items: {
          oneOf: [
            {
              ...
              properties: {
                ...
                environment: { const: 'sandbox' },
                provider: { const: 'apollo-sandbox-fake' },
                operation: { const: 'semantic-embedding' },
                inputHash: sha256Schema,
                outputHash: sha256Schema,
                units: { type: 'integer', minimum: 1, maximum: 100000 },
                cost: {
            ...
            },
            {
              ...
              properties: {
                schemaVersion: { const: 'sandbox-provider-receipt/v2' },
                workspaceId: idSchema,
                clientId: idSchema,
                environment: { const: 'sandbox' },
                provider: { const: 'apollo-sandbox-fake' },
                operation: { enum: ['semantic-embedding', 'transcription', 'speaker-diarization'] },
                inputHash: sha256Schema,
                outputHash: sha256Schema,
                units: { type: 'integer', minimum: 1, maximum: 100000 },
                cost: {
  ...
  defineSchema('register-synthetic-presenter-request', 1, 'Register one immutable consent-bound synthetic presenter profile version', {
    type: 'object', additionalProperties: false,
    required: ['profileId', 'version', 'actorIdentityId', 'avatar', 'voice', 'defaultLocale', 'status', 'disclosure', 'consent'],
    properties: {
      profileId: idSchema, version: { type: 'integer', minimum: 1 }, actorIdentityId: idSchema,
      ...
      defaultLocale: { type: 'string', minLength: 2, maxLength: 35 }, status: { enum: ['active', 'disabled', 'expired'] }, disclosure: { type: 'string', minLength: 3, maxLength: 500 },
      consent: {
        type: 'object', additionalProperties: false,
  ...
  defineSchema('create-synthetic-production-run-request', 1, 'Compile approved synthetic media into one immutable EditPlan', {
    ...
    properties: {
      ...
      audio: {
        type: 'object', additionalProperties: false,
        required: ['artifactId', 'durationMs', 'locale', 'scriptHash', 'alignment'],
        properties: {
          artifactId: idSchema, durationMs: { type: 'integer', minimum: 1 }, locale: { type: 'string', minLength: 2, maxLength: 35 }, scriptHash: sha256Schema,
          alignment: { type: 'array', minItems: 1, maxItems: 100000, items: { type: 'object', additionalProperties: false, required: ['text', 'startMs', 'endMs'], properties: { text: { type: 'string', minLength: 1, maxLength: 500 }, startMs: { type: 'integer', minimum: 0 }, endMs: { type: 'integer', minimum: 1 } } } },
        },
      },
      blocks: {
        type: 'array', minItems: 1, maxItems: 500,
        items: { type: 'object', additionalProperties: false, required: ['id', 'text', 'rangeMs', 'cacheKey', 'providerJobId', 'audioSha256', 'artifactId', 'critic'], properties: {
          id: idSchema, text: { type: 'string', minLength: 1, maxLength: 10000 }, rangeMs: syntheticRangeSchema, cacheKey: sha256Schema, providerJobId: idSchema, audioSha256: sha256Schema, artifactId: idSchema,
          critic: { type: 'object', additionalProperties: false, required: ['id', 'resultHash', 'status'], properties: { id: idSchema, resultHash: sha256Schema, status: { enum: ['approved', 'rejected'] } } },
        } },
      },
      bRoll: { type: 'array', maxItems: 500, items: { type: 'object', additionalProperties: false, required: ['id', 'rangeMs', 'artifactId'], properties: { id: idSchema, rangeMs: syntheticRangeSchema, artifactId: idSchema } } },
      overlays: { type: 'array', maxItems: 500, items: { type: 'object', additionalProperties: false, required: ['id', 'rangeMs', 'artifactId'], properties: { id: idSchema, rangeMs: syntheticRangeSchema, artifactId: idSchema } } },
      captions: { type: 'boolean' }, use: idSchema, market: { type: 'string', minLength: 2, maxLength: 64 },
    },
  }),
  defineSchema('synthetic-production-run-mutated', 1, 'Created or replayed synthetic production run',
    successSchema({ type: 'object', additionalProperties: false, required: ['run', 'replayed'], properties: { run: { type: 'object', additionalProperties: false, required: ['id', 'status', 'editPlanSnapshotId', 'plan'], properties: { id: idSchema, status: { enum: ['compiled', 'rendering', 'completed', 'failed', 'canceled'] }, editPlanSnapshotId: idSchema, plan: syntheticProductionPlanSchema } }, replayed: { type: 'boolean' } } }),
  ),
  ...
  // -------------------------------------------------------------------------
  // Wave 18 — F4.001-F4.008: capture sessions and multi-range synthesis.
  //
  // Every tick crosses this boundary as a DECIMAL STRING, never as a JSON
  // number. A JSON number is an IEEE 754 double in every mainstream parser, and
  // a double holds every integer only up to 2^53 — so a nanosecond tick would
  // arrive at a JavaScript client already rounded, with no error raised and no
  // way to tell. Rates cross as "num/den" for the same reason: 30000/1001 has
  // no decimal form, and a client handed 29.97 could never recover it.
  // -------------------------------------------------------------------------
  defineSchema(
    'create-capture-session-request',
    1,
    'Open a capture session around one reference track',
    {
      type: 'object',
      additionalProperties: false,
      required: ['sessionId', 'clock', 'referenceTrack', 'lineage'],
      properties: {
        sessionId: idSchema,
        clock: captureClockPolicySchema,
        referenceTrack: captureTrackInputSchema,
        lineage: captureCommandLineageSchema,
      },
    },
  ),
  defineSchema(
    'add-capture-track-request',
    1,
    'Add one recorder to an existing capture session',
    {
      type: 'object',
      additionalProperties: false,
      required: ['baseVersionId', 'baseHash', 'track', 'lineage'],
      properties: {
        // The chain is content-addressed, so a caller names both the version it
        // read and that version's hash. A version number alone can be reused
        // after a failed write; the pair cannot, so a command computed against
        // a session that has moved is refused rather than applied.
        baseVersionId: idSchema,
        baseHash: sha256Schema,
        track: captureTrackInputSchema,
        lineage: captureCommandLineageSchema,
      },
    },
  ),
  defineSchema(
    'add-capture-track-part-request',
    1,
    'Add one more file to a track already in the session',
    {
      type: 'object',
      additionalProperties: false,
      required: ['baseVersionId', 'baseHash', 'trackId', 'part', 'lineage'],
      properties: {
        // The chain is content-addressed, so a caller names both the version it
        // read and that version's hash. A version number alone can be reused
        // after a failed write; the pair cannot, so a command computed against
        ...
        baseHash: sha256Schema,
        trackId: idSchema,
        part: captureTrackPartInputSchema,
        lineage: captureCommandLineageSchema,
      },
    },
  ),
  defineSchema(
    'change-capture-reference-track-request',
    1,
    'Point the session at a different reference track',
    {
      type: 'object',
      additionalProperties: false,
      required: ['baseVersionId', 'baseHash', 'referenceTrackId', 'lineage'],
      properties: {
        // The chain is content-addressed, so a caller names both the version it
        // read and that version's hash. A version number alone can be reused
        // after a failed write; the pair cannot, so a command computed against
  ...
  defineSchema(
    'request-capture-sync-request',
    1,
    'Ask for a capture session to be synchronized',
    {
      type: 'object',
      additionalProperties: false,
      required: ['baseVersionId', 'baseHash'],
  ...
  defineSchema(
    'capture-session-mutated',
    1,
    'The session version a capture command produced',
    successSchema({
      type: 'object',
      additionalProperties: false,
      required: ['session', 'replayed'],
      properties: { session: captureSessionSummarySchema, replayed: { type: 'boolean' } },
    }),
  ),
  defineSchema(
    'capture-sync-run-mutated',
    1,
    'The durable operation a sync request started',
    successSchema({
      type: 'object',
      additionalProperties: false,
      required: ['run', 'replayed'],
      properties: {
        run: {
          type: 'object',
          additionalProperties: false,
          required: ['sessionId', 'operationId', 'state', 'sessionVersion'],
          properties: {
            sessionId: idSchema,
            operationId: idSchema,
  ...
  defineSchema(
    ...
    1,
    'One version of a capture session, with its tracks',
    successSchema({
      type: 'object',
      additionalProperties: false,
      required: ['session'],
  ...
  defineSchema(
    ...
    1,
    'Capture sessions of one project, newest first',
    successSchema({
      type: 'object',
      additionalProperties: false,
      required: ['sessions'],
      properties: {
        sessions: { type: 'array', items: captureSessionSummarySchema },
      },
    }),
  ),
  defineSchema(
    'capture-session-version-list',
    1,
    'The immutable chain of a capture session, newest first',
    successSchema({
      type: 'object',
      additionalProperties: false,
      required: ['sessionId', 'versions'],
      properties: {
        sessionId: idSchema,
        versions: {
          type: 'array',
          items: {
            type: 'object',
            additionalProperties: false,
            required: ['version', 'sessionHash', 'operation', 'actorKind', 'actorId', 'occurredAt'],
            properties: {
              version: { type: 'integer', minimum: 1 },
              previousVersionHash: { type: ['string', 'null'], pattern: '^[a-f0-9]{64}$' },
              sessionHash: sha256Schema,
  ...
  defineSchema(
    'capture-sync-read',
    1,
    'What the evidence cascade decided for every track of a session',
    successSchema({
      type: 'object',
      additionalProperties: false,
      required: ['sessionId', 'sessionVersion', 'referenceTrackId', 'tracks'],
      properties: {
        sessionId: idSchema,
        sessionVersion: { type: 'integer', minimum: 1 },
        referenceEpoch: { type: 'integer', minimum: 1 },
        referenceTrackId: idSchema,
        tracks: { type: 'array', items: captureSyncTrackSchema },
      },
    }),
  ),
  defineSchema(
    'create-editorial-synthesis-request',
    1,
    'Assemble several source ranges into one cut',
    {
      type: 'object',
      additionalProperties: false,
      required: ['synthesisId', 'objective', 'targetDurationMs', 'toleranceMs', 'sourceDurationMs',
        'frameRate', 'storyPlanId', 'editPlanId', 'ranges', 'joins'],
      properties: {
        synthesisId: idSchema,
        objective: { type: 'string', minLength: 1, maxLength: 512 },
        targetDurationMs: { type: 'integer', minimum: 1_000, maximum: 3_600_000 },
        toleranceMs: { type: 'integer', minimum: 0, maximum: 60_000 },
        sourceDurationMs: { type: 'integer', minimum: 1, maximum: 86_400_000 },
        frameRate: rationalSchema,
        storyPlanId: idSchema,
        editPlanId: idSchema,
        // Declared only when the cut departs from source order. Reordering
        // changes what the material asserts about cause, so it is never
        // inferred from the array happening to arrive out of order.
        allowReorder: {
          type: 'object',
          additionalProperties: false,
          required: ['reason'],
          properties: { reason: { type: 'string', minLength: 12, maxLength: 512 } },
        },
        ranges: {
          type: 'array',
          minItems: 1,
          maxItems: 256,
          items: synthesisRangeInputSchema,
        },
        joins: {
          type: 'array',
          maxItems: 255,
          items: synthesisJoinInputSchema,
        },
      },
    },
  ),
  defineSchema(
    'editorial-synthesis-mutated',
    1,
    'The cut a multi-range synthesis command produced',
    successSchema({
      type: 'object',
      additionalProperties: false,
      required: ['synthesis', 'replayed'],
      properties: { synthesis: editorialSynthesisSummarySchema, replayed: { type: 'boolean' } },
    }),
  ),
  defineSchema(
    'editorial-synthesis-read',
    1,
    'One persisted multi-range cut, with the proof its context survived',
    successSchema({
      type: 'object',
      additionalProperties: false,
      required: ['synthesis'],
      properties: { synthesis: editorialSynthesisSchema },
    }),
  ),
  defineSchema(
    'editorial-synthesis-list',
    1,
    'Multi-range cuts of one project, newest first',
    successSchema({
      type: 'object',
      additionalProperties: false,
      required: ['syntheses'],
