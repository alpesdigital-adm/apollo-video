...
import { parseAttachConsentBody, presentPresenterProfile } from '@/v2/public-api/synthetic-presenter-lifecycle-contract'
...
export async function POST(request: NextRequest, context: { params: Promise<{ workspaceId: string; presenterId: string }> }) {
  ...
  try {
    ...
    const body = parseAttachConsentBody(raw)
    const repository = createSyntheticProductionRepository()
    const result = await attachSyntheticPresenterConsentProofService({
