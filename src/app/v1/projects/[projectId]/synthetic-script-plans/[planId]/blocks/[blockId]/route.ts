...
export async function DELETE(request: NextRequest, context: { params: Promise<{ projectId: string; planId: string; blockId: string }> }) {
  ...
  try {
    ...
    const body = parseRemoveBlockBody(raw)
    ...
    const mutated = await services.mutatePlan({
      ...
      mutation: { kind: 'remove-block', blockId },
      actor,
