...
export async function POST(request: NextRequest, context: { params: Promise<{ projectId: string; runId: string }> }) { const requestId = resolveRequestId(request); try { const actor = await authenticateExternalRequest(request); requireScope(actor, 'projects:write'); assertExternalMutationOrigin(request, actor); const { projectId, runId } = await context.params; const run = await createMusicAnalysisRuntime().runs.retry({ workspaceId: actor.workspaceId, projectId, runId, now: new Date().toISOString(), authenticationAudit: materializeActorAuditContext(actor) }); return NextResponse.json(presentSuccess({ run: presentMusicAnalysisRun(run) }), { status: 202, headers: publicApiHeaders(requestId) }) } catch (error) { return respondPublicError(error, requestId) } }
