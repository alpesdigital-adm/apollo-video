...
async function parseBody(request: NextRequest) {
  ...
  if (
    ...
    Object.keys(body).toSorted().join(',') !== 'baseRevision,confirmed'
  ) {
    throw new DomainError('INVALID_ARGUMENT', 'Project archive fields are invalid')
  }
  const value = body as Record<string, unknown>
  if (!Number.isSafeInteger(value.baseRevision) || value.confirmed !== true) {
    throw new DomainError('INVALID_ARGUMENT', 'Project archive values are invalid')
  }
