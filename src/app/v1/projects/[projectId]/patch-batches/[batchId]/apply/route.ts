...
import { applyReviewPatchBatchService } from '@/v2/application/review-patch-batch'
...
export async function POST(request: NextRequest, context: { params: Promise<{ projectId: string; batchId: string }> }) {
  ...
  try {
    ...
    if (Object.keys(record).some((key) => key !== 'confirmed') || record.confirmed !== true) throw new DomainError('PRECONDITION_REQUIRED', 'Batch patch impact must be explicitly confirmed')
    const { projectId, batchId } = await context.params
    const repository = createReviewPatchBatchRepository()
    const result = await applyReviewPatchBatchService({
      repository,
      clock: () => new Date(),
