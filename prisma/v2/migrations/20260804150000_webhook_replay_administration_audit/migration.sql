...
ADD CONSTRAINT "webhook_admin_commands_target_check" CHECK (
  ...
  ("targetType" = 'webhook-signing-secret-rotation' AND "action" LIKE 'webhook-signing-secret-rotation.%' AND "endpointId" IS NOT NULL) OR
  ("targetType" = 'webhook-delivery' AND "action" = 'webhook-delivery.replay' AND "endpointId" IS NULL) OR
  ("targetType" = 'webhook-event' AND "action" = 'webhook-event.replay' AND "endpointId" IS NULL)
),
