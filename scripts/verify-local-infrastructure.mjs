...
const storageDockerfile = read('infra/object-storage/Dockerfile')
...
assert.deepEqual([...storage.matchAll(/^\s+image:\s+(\S+)$/gm)].map((match) => match[1]), [versions.storage.minioImage, versions.storage.minioImage], 'MinIO server and initializer must use the same local source-built image')
assert.equal((storage.match(/^\s+build:$/gm) ?? []).length, 2, 'Both MinIO services must declare the source build')
assert.equal((storage.match(/^\s+context: \.\.\/object-storage$/gm) ?? []).length, 2, 'Both MinIO builds must resolve to object-storage with isolated and combined Compose files')
assert.equal((storage.match(/^\s+dockerfile: Dockerfile$/gm) ?? []).length, 2, 'Both MinIO services must use the pinned Dockerfile')
assert.equal((storage.match(/^\s+pull_policy: build$/gm) ?? []).length, 2, 'Both MinIO services must build rather than pull an image')
forbids(storage, /quay\.io\/minio|minio\/minio:|pull_policy: (?:always|missing|never)/, 'Object storage must not pull a MinIO runtime image')
forbids(storageDockerfile, /FROM (?:quay\.io\/minio|minio\/minio)|(?:^|\n)\s*(?:RUN|ADD)\s+.*(?:apk add|apt-get|@latest)/, 'Source build must not reintroduce an upstream runtime image or floating packages')
