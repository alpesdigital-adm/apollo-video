...
> **Wave 22 (2026-09-11) — nota, sem caixa marcada.** A página do editor passou a expor o resultado de uma leitura falha como estado próprio, não como aviso descartável: um 409 `PERSISTENCE_CONFLICT` na leitura de annotations renderiza `review-unavailable` com código e request id, mantém o preview já autorizado inspecionável e desabilita `review-save`, `review-patch-apply`, `review-batch-apply` e `review-batch-prepare`; um 429 mostra quando a nova tentativa abre e as leituras automáticas respeitam o mesmo instante; 401 encerra o ciclo e leva ao login. As leituras do editor passam por `src/app/_operator/editor-reads.ts` (dedup em voo, resposta atrasada descartada após troca de projeto/sessão, polling que aguarda a própria rodada e pausa com a aba oculta), provado por `tests/v2/editor-reads.test.mjs` (20 casos com rede e relógio falsos). O E2E de navegador `tests/v2/editor-reliability-browser.e2e.mjs` é passo nomeado do CI (`test:e2e:editor-reliability`). Deploy, execução visual e aceite do proprietário continuam abertos; a recuperação das nove annotations históricas sem auditoria segue fora de escopo.
