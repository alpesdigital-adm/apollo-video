...
- [ ] Criar seeds mínimos para workspace, projeto, source e OutputSpec. Parcial F0.030: o bootstrap real de workspace/client foi complementado por um seed idempotente que cria Project + OutputSpec no snapshot imutável, atravessa upload V2 de um master real com hash/tamanho/rights verificados, aguarda o ingest durável e só retorna sucesso após comprovar os relacionamentos source-master e editing-proxy persistidos. Parser, limites e replay do upload verificado têm cobertura local; PostgreSQL/worker/FFmpeg não puderam ser executados neste host, portanto o seed integrado permanece aberto. Evidência: T-F0.030 e ADR-134.
- [ ] Configurar audit, typecheck, unit, migration check, contract gate, build e integrações Postgres/API no CI. Parcial F0.030: o workflow contém esses gates, usa a mesma imagem PostgreSQL 16 + pgvector exigida pelas migrations, bloqueia drift da infraestrutura e executa os três conjuntos de goldens FFmpeg descobertos explicitamente. O run hospedado `30755105914` passou por audit, typecheck, lint, contratos, migrations, build, integrações PostgreSQL e 9 goldens, mas revelou no E2E API que a hidratação de impacto LUT comparava a ordem de chaves do objeto em vez do JSON canônico persistido; a regressão foi corrigida localmente e ainda exige novo run integralmente verde. Evidência: `.github/workflows/ci.yml`.
- [ ] Configurar lint no CI após selecionar regras e corrigir o baseline da v1. Parcial F0.030: ESLint 9 + configuração oficial Next 16 core-web-vitals/TypeScript cobre `src`, `scripts` e `tests` com zero warnings permitido; o baseline local de 128 achados foi reduzido a zero sem esconder regras estáveis. Regras experimentais do React Compiler e `unused/explicit-any/empty-object` foram excluídas explicitamente por incompatibilidade com os padrões canônicos existentes; navegação interna, dependências de hooks e erros objetivos foram corrigidos. O gate passou no Linux hospedado do run `30755105914`; o run integral falhou posteriormente no E2E API e ainda não autoriza marcar a caixa. Evidência: T-F0.030 e ADR-134.
- [ ] Configurar golden tests no CI após existir fixture/render determinístico. Parcial F0.030: o workflow executa renderer editorial (6 casos), transcript retimado via worker (2 casos) e extração contígua de duas horas (1 caso), todos com FFmpeg real e asserts de frames/pixels/áudio; o golden long-form exige a compilação de cor V2. Os 9 casos passaram localmente e no Linux hospedado do run `30754522174`; o run falhou somente depois, na integração API por drift da constraint de Command agora corrigido. Falta consolidar a política final de tolerância cross-platform e obter o run corretivo integralmente verde. Evidência: T-F0.030 e ADR-134.
- [ ] Configurar E2E no CI após existir jornada vertical F0 executável. Evidência: T-F0.030 e ADR-134.
- [ ] Criar telemetria comum com trace, job, workspace e project IDs. Parcial F0.030: o runtime criado pela repository factory agora envolve todas as operações públicas com um port de telemetria e um adapter JSON estruturado. Eventos de criação/replay, claim, heartbeat, avanço de fase, espera/retomada, retry, cancelamento, sucesso e falha carregam schema versionado, trace determinístico por workspace+operação, job ID, workspace, project quando o contexto possui vínculo, tipo/status/fase/attempt; nenhum payload, nome de arquivo, transcript ou identidade de artifact é aceito no envelope. Falha síncrona ou assíncrona do coletor não altera a operação durável. Testes locais provam a jornada de worker e a allowlist, mas ainda faltam propagação do trace de requisição entre processos/providers/renderer, métricas completas, execução hospedada e aceite; a caixa e T-NFR-003 permanecem abertos. Evidência: T-F0.030 e ADR-134.
