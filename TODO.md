...
- [ ] Vincular client, workspace e delegated user ao audit context. Parcial local F0.036: Bearer e sessão humana constroem o mesmo `ExternalAuditContext` congelado e todo `requireScope` revalida client, credential, workspace, environment e delegação antes de autorizar. Comandos de containment, administração de clients/credentials, as onze mutações administrativas de webhook e a transição pública do lifecycle de media artifact persistem internamente o tuple completo e seu hash canônico, sem ampliar responses públicas. Todas exigem o ator no Application service e gravam intenção+efeito na mesma transação; operações idempotentes incorporam o hash ao fingerprint, enquanto challenge só grava autoria junto da verificação+ativação efetiva e seguidores exigem o mesmo command. A transição de artifact também revalida o tuple na hidratação, impede replay entre credentials/members e remove no reset pré-produção linhas/replays antigos sem autoria reconstruível. PostgreSQL/HTTP hospedado, as demais famílias do ledger, deploy e aceite permanecem abertos. Evidência: T-FR-236, T-FR-242, T-FR-244 e ADR-134.
