...
- [ ] Enfileirar proxy/range render mínimo e manter outputs válidos. Parcial F0-027: `apollo.projects.manual-edits.apply` v2 continua enfileirando o worker V2 existente, que deriva do Command persistido um único range stale, recompõe apenas esse trecho e monta um MP4 completo reutilizando prefixo/sufixo do proxy concluído da versão-base, com validação de hash, tamanho, duração e lineage. Seleção sem mudança de render conclui a própria operação por cache hit do proxy-base exato, sem criar artifact, resolver cor ou chamar worker; Postgres relê Command/impact/versões/proxy/artifact/manifest e persiste `reusedFromOperationId` com FKs. `apollo.projects.commands.apply` enfileira proxy integral idempotente para `remove-spoken-content` e `run-director`. `apollo.projects.lut-selection.set` v2 usa o mesmo application service para enfileirar um proxy full-timeline da versão-resultado quando a timeline é renderizável; antes disso persiste a seleção e devolve o deferimento sem operação. Ainda faltam integrar o slice multi-range/rate, outros Command types, E2E PostgreSQL executado, deploy e aceite. Evidência: T-FR-233 e ADR-133.
