...
- [ ] Definir event envelope versionado, IDs únicos e catálogo inicial. Evidência F0-032/F0-038 local: `PublicEvent`, UUID v4, catálogo de 14 tipos, schemas e `GET /v1/events/catalog`; o outbox usa o ID como chave primária global. Treze tipos possuem ao menos um producer transacional real; `budget.threshold.reached` permanece sem producer até existir ledger/threshold canônico. Deploy e aceite continuam pendentes.
- [ ] Implementar outbox transacional a partir de domain/workflow transitions. Parcial local F0-038: além de `project.created` e `project.version.created`, toda mudança canônica de status feita pelo repositório geral de `PublicOperation` grava `operation.status.changed` e, quando aplicável, `operation.succeeded`/`operation.failed` na mesma transação. Criação de long-form/source-cleanup, conclusão do Diretor e cancelamento em massa por controle de acesso também usam o mesmo writer; replay/convergência não duplicam eventos e o payload omite lease, autorização, storage e erro interno. Um writer comum passou a persistir também `annotation.created`, `annotation.resolved`, `artifact.ready`, `artifact.rejected` e `client.suspended` no commit que vence a respectiva transição, sem texto de review, screenshot ou reason sensível. `budget.threshold.reached`, cobertura exaustiva de todo writer futuro, execução PostgreSQL hospedada, deploy e aceite permanecem abertos. Evidência: T-FR-244 e ADR-134.
