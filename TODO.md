...
- [ ] Definir estados válidos de projeto, versão, job, item batch e artifact. Parcial F0.029: `Project` agora possui enum fechado para as 14 fases e uma projeção visível canônica, além das projeções já fechadas de `PublicOperation`, `ProductionBatch`, itens, invalidation version-scoped e artifact. A integração do projeto ainda cobre somente create/list; workspace e estado de versão permanecem abertos. Evidência: T-FR-236 e ADR-133.
- [ ] Implementar transições server-side e rejeitar saltos inválidos. Parcial F0.029: `PublicOperation` ganhou transições puras `running → waiting → running`, preservando attempt/progresso e recusando espera fora de execução ou retomada para fase anterior. Project e artifact falham fechado na hidratação/apresentação de estado desconhecido, mas as mutações de Project ainda estão dispersas e não possuem uma matriz canônica de transições; persistência/lease de waiting e transições explícitas de artifact também permanecem abertas. Evidência: T-FR-236 e ADR-133.
- [ ] Mapear estado técnico para label, progresso e ação na UI. Parcial F0.029: operações v7, batch v2, invalidations v2, `apollo.artifacts.read` v3, `apollo.projects.list` v2 e `apollo.projects.create` v3 retornam projeções visíveis aditivas; versões anteriores permanecem publicadas. As leituras de workspace ainda não projetam o estado de Project. Evidência: T-FR-236 e ADR-133.
- [ ] Testar sucesso, espera, retry, cancel, falha parcial, stale e conclusão. Parcial F0.029: operações cobrem sete estados; batch cobre falha parcial; invalidation cobre stale relacional; artifact cobre seu lifecycle; Project cobre as 14 fases, imutabilidade, correspondência status/label/ação e estado inventado fail-closed em create/list. Matriz de transições persistidas, workspace, PostgreSQL E2E, deploy e aceite permanecem abertos. Evidência: T-FR-236 e ADR-133.
