...
- [ ] Vincular client, workspace e delegated user ao audit context. Parcial local F0.036: Bearer e sessão humana constroem o mesmo `ExternalAuditContext` congelado a partir de identidade autenticada; UI inclui member, login identity e role duráveis, enquanto Bearer não pode inventar delegação. Todo `requireScope` revalida igualdade de client, credential, workspace, environment e campos delegados entre actor e audit context antes de autorizar. A coleção de scopes agora é um `ReadonlySet` real sem métodos mutadores. Comandos de containment persistem separadamente o tuple completo client+credential/session+environment+authentication kind+member+identity+role e seu hash canônico, sem expô-lo na response; replay administrativo pelo mesmo client UI mas por outra identidade delegada falha fechado. A migration destrutiva remove commands antigos impossíveis de atribuir honestamente em vez de inventar backfill. PostgreSQL/HTTP hospedado, ledger exaustivo para as demais capabilities, deploy e aceite permanecem abertos. Evidência: T-FR-242 e ADR-134.
...
- [ ] Criar security E2E de scope, cross-workspace, expiry, rotation e revocation. Parcial local F0.036: `public-project-api.integration.mjs` agora cobre capability filtering, 403/404 indistinguível, negação de admin para reviewer sobre o client UI compartilhado, self suspend/revoke sem command persistido, expiração Bearer sem atualizar last-use, rotação concorrente com overlap zero, token antigo e credential revogada. A jornada também lê as linhas reais de containment para provar audit Bearer sem delegação, recovery UI com member+identity+role e hashes distintos. Ela usa API HTTP, Next de produção e PostgreSQL reais, mas esta ampliação ainda não foi executada no ambiente isolado hospedado; deploy e aceite permanecem pendentes.
...
- [ ] Criar administração de webhooks, subscriptions e delivery diagnostics. Evidência local F0-038: a API externa cobre endpoint create/list/read/status/challenge, subscription create/list/read/status, signing-secret lifecycle, delivery list/read/replay e paginação diagnóstica; a UI de configurações consome exclusivamente essas rotas, apresenta timeline de attempts, lifecycle reversível, subscriptions, rotação e replay. O gate estrutural fixa autenticação, catálogo e paridade das 15 ações. E2E real em browser/PostgreSQL, operações terminais, deploy e aceite ainda impedem fechar a caixa.
