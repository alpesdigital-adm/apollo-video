...
- [ ] Mapear estado técnico para label, progresso e ação na UI. Parcial F0.029: operações v7, batch v2, invalidations v2, artifact v3, project create/list/workspace e `apollo.projects.annotations.list` v3 retornam projeções visíveis aditivas; versões anteriores permanecem publicadas. ProjectVersion atual abre o resultado; superseded abre preview histórico somente quando disponível. Evidência: T-FR-236 e ADR-133.
- [ ] Testar sucesso, espera, retry, cancel, falha parcial, stale e conclusão. Parcial F0.029: além das coberturas anteriores, ProjectVersion prova current/superseded, preview histórico disponível/ausente, imutabilidade e adulteração de contrato. PostgreSQL E2E dos demais writers, projeção de versão nas demais respostas, transições de artifact, deploy e aceite permanecem abertos. Evidência: T-FR-236 e ADR-133.
