...
- [ ] Configurar golden tests no CI após existir fixture/render determinístico. Parcial F0.030: o workflow agora executa renderer editorial (6 casos), transcript retimado via worker (2 casos) e extração contígua de duas horas (1 caso), todos com FFmpeg real e asserts de frames/pixels/áudio; o golden long-form foi atualizado para exigir a compilação de cor V2. Os 9 casos passam localmente, mas a execução Linux hospedada e a política final de tolerância cross-platform ainda pendem. Evidência: T-F0.030 e ADR-134.
