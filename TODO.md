...
- [ ] Mapear cada command aos ranges, variants e artifacts afetados. Parcial F0-027: `manual-edit` persiste `command-impact/v1` content-addressed no payload v2 do mesmo `V2EditCommand`, com ranges frame-first, formato configurado, dependências, outputs proxy/final concluídos da versão-base e render mínimo esperado. `remove-spoken-content` persiste `editorial-cut-impact/v1` e `run-director` agora persiste `director-run-impact/v1`; como ambos podem alterar a composição integral, declaram full-timeline, todos os outputs concluídos da base e exatamente um proxy integral do formato corrente. O Diretor inclui áudio/conteúdo/policy/timing/visual e vincula transcript/planner/critic. Os três commits relêem os outputs na transação e falham se o conjunto mudar. Demais command types e execução multi-range/rate continuam abertos. Evidência: T-FR-233 e ADR-133.
- [ ] Marcar somente dependentes como stale. Parcial F0-027: os commits serializáveis de `manual-edit`, `remove-spoken-content` e `run-director` gravam `command-artifact-invalidation/v1` normalizado somente para os outputs exatos do mapa, ligados a Command, versão-base, versão-resultado e artifact; sem output-base concluído nenhum deles fabrica linha stale. Bytes históricos mantêm `V2MediaArtifact.status=available`. A conclusão do proxy grava uma resolução imutável ligada à operação e aos artifacts substitutos; a capability `apollo.projects.artifact-invalidations.read` deixa de listar a relação somente quando essa operação chega a `succeeded`. O E2E PostgreSQL/API do Diretor foi preparado, mas não executado neste host. Faltam os demais Command types, execução PostgreSQL, deploy e aceite. Evidência: T-FR-233 e ADR-133.
- [ ] Enfileirar proxy/range render mínimo e manter outputs válidos. Parcial F0-027: `apollo.projects.manual-edits.apply` v2 continua enfileirando o worker V2 existente, que deriva do Command persistido um único range stale, recompõe apenas esse trecho e monta um MP4 completo reutilizando prefixo/sufixo do proxy concluído da versão-base, com validação de hash, tamanho, duração e lineage. Seleção sem mudança de render conclui a própria operação por cache hit do proxy-base exato, sem criar artifact, resolver cor ou chamar worker; Postgres relê Command/impact/versões/proxy/artifact/manifest e persiste `reusedFromOperationId` com FKs. `apollo.projects.commands.apply` agora enfileira um proxy integral idempotente tanto para `remove-spoken-content` quanto para `run-director`, usando a versão-resultado como fence; nenhum tenta reuso parcial porque recompila ou redireciona o plano inteiro. Sem proxy-base concluído a seleção falha fechada, e sem proxy reutilizável ou quando todo o timeline é afetado executa render V2 completo. Ainda faltam múltiplos ranges/rates, outros Command types, E2E PostgreSQL executado, deploy e aceite. Evidência: T-FR-233 e ADR-133.
