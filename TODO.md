...
- [ ] Salvar manifest com checksums, plan hash e renderer version. Parcial F0.028: `media-artifact-manifest/v4` vincula por hash um `render-input/v1` protegido que contém checksums ordenados, plan hash e identidade versionada do renderer; a criação agora exige correspondência bidirecional, ordenada e por role entre todas as sources e todos os assets não-LUT, impedindo font/data/mídia consumidos sem edge de lineage. LUT permanece fora dessas edges e vinculada pela identidade versionada especializada. Faltam golden reconstruído da fixture persistida, PostgreSQL/MinIO reais, deploy e aceite. Evidência: T-FR-234 e ADR-133.
