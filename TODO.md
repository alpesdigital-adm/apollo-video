...
- [ ] Definir estados válidos de projeto, versão, job, item batch e artifact. Parcial F0.029: `Project` possui enum fechado para as 14 fases; `ProjectVersion` deriva `current|superseded` da relação imutável com `currentVersionId`; operações, batch/itens, invalidation e artifact também possuem projeções fechadas. A versão está integrada no histórico de review, create, duplicate, workspace, Commands, manual edit, patches e LUT selection; compare restore permanece aberto. Evidência: T-FR-236 e ADR-133.
- [ ] Implementar transições server-side e rejeitar saltos inválidos. Parcial F0.029: `Project` possui matriz canônica exaustiva para as 14 fases; todos os writers atuais de ingest, proxy review, compare e export final cercam o `UPDATE` com os estados de origem permitidos e fazem rollback por conflito quando o fence não encontra a origem esperada. `PublicOperation` preserva attempt/progresso em `running → waiting → running`. Persistência/lease de waiting, writers futuros para fases ainda não executadas e transições explícitas de artifact/versão permanecem abertas. Evidência: T-FR-236 e ADR-133.
- [ ] Mapear estado técnico para label, progresso e ação na UI. Parcial F0.029: operações v7, batch v2, invalidations v2, artifact v3, project list v4, create v4, duplicate v2, workspace v7, review v3, commands v7/result v6, manual edit v3, patch individual/batch v3 e LUT read/set v3 retornam projeções visíveis aditivas; versões anteriores permanecem publicadas. ProjectVersion atual abre o resultado; superseded abre preview histórico somente quando disponível. Evidência: T-FR-236 e ADR-133.
- [ ] Testar sucesso, espera, retry, cancel, falha parcial, stale e conclusão. Parcial F0.029: além das coberturas anteriores, ProjectVersion prova current/superseded, preview histórico disponível/ausente, create/duplicate/workspace, Commands, ações manuais, patches e LUT renderable/deferred, com imutabilidade e adulteração de contrato. PostgreSQL E2E dos demais writers, compare restore, transições de artifact, deploy e aceite permanecem abertos. Evidência: T-FR-236 e ADR-133.
