...
Incremento local F0.102: as cinco rotas operáveis de busca semântica passaram a resolver o provider pelo environment autenticado, não por fallback global. Em `sandbox`, a seleção é invariavelmente `apollo-sandbox-fake`, mesmo quando OpenAI está configurado; cada embedding determinístico grava receipt content-addressed com hashes de input/output, unidades, custo inteiro simulado e `externalCalls=0`. Em `production`, fake/deterministic é rejeitado e OpenAI sem chave falha fechado. A capability administrativa `apollo.governance.sandbox-executions.list` pagina somente receipts redigidos e workspace-bound. A jornada PostgreSQL/HTTP de hybrid search foi convertida para provar esse roteamento e a consulta pública, mas permanece não executada neste host; transcrição, diarização e demais providers ainda precisam do mesmo router, além de deploy e aceite, portanto F0.102 continua aberta.
...
### Incremento operacional F0.102 — sandbox roteado na busca semântica

**Status:** implementado e testado localmente em 5 de agosto de 2026; E2E PostgreSQL/HTTP preparado, sem execução, deploy ou aceite.

- o environment do ator autenticado é obrigatório no composition root das cinco rotas semânticas; configuração global não pode promover sandbox a provider externo;
- sandbox usa projeção determinística e persiste um receipt SHA-256 por intenção/output, com custo simulado inteiro e zero chamadas externas; replay converge pela identidade content-addressed;
- production rejeita provider deterministic/fake e exige configuração explícita válida da OpenAI, sem fallback silencioso;
- `GET /v1/governance/sandbox-executions` exige `clients:admin`, workspace binding e cursor canônico, expondo somente hashes, uso, custo simulado e prova `externalCalls=0`;
- migration impõe environment/provider/operação/custo/zero-call por constraints e FKs compostas ligam cada receipt ao client do workspace;
...
- transcriber, diarization e providers sintéticos continuam fora deste router, portanto F0.102 permanece aberta e nenhuma caixa foi marcada.
