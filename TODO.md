...
- [ ] Criar endpoints de list/read/cancel/retry e filtros por projeto/status/type. Parcial F0.037 local: list/read/cancel/retry já existentes evoluíram para contratos v8 e agora aceitam `projectId`, vinculado canonicamente e indexado em `PublicOperation`; o cursor inclui o filtro no hash e troca de projeto entre páginas falha fechada. Ingest, proxy, export, cleanup e long-form exigem projeto no domínio, banco e reidratação; `artifact-render` permanece explicitamente global. Migração, E2E PostgreSQL e HTTP estão preparados, mas ainda faltam execução hospedada, integração em `main`, deploy e aceite; por isso a caixa permanece aberta. Evidência: T-FR-243 e ADR-019.
