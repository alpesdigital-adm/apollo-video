...
- [ ] Criar telemetria comum com trace, job, workspace e project IDs. Parcial F0.030: o runtime criado pela repository factory envolve todas as operações públicas com um port de telemetria e adapter JSON estruturado. O `Apollo-Request-Id` validado agora atravessa ingest, render autorizado, proxy, export, cleanup e long-form, é persistido em PostgreSQL na operação durável e reaparece no worker após claim/replay; operações anteriores ou internas mantêm fallback determinístico por workspace+operação. Eventos de criação/replay, claim, heartbeat, fase, espera/retomada, retry, cancelamento, sucesso e falha carregam trace, job ID, workspace, project quando vinculado, tipo/status/fase/attempt, sem payload, nome de arquivo, transcript, URL ou identidade de artifact. O run hospedado `30761608326` passou migration limpa, 796 testes, integrações Prisma/API, build, smoke vertical e topologia Compose estável. Ainda faltam spans do mesmo trace dentro de providers/renderer, métricas completas, dashboards e aceite; a caixa e T-NFR-003 permanecem abertos. Evidência: T-F0.030 e ADR-134.
