...
- [ ] Implementar transições server-side e rejeitar saltos inválidos. Parcial F0.029: `Project` possui matriz canônica exaustiva para as 14 fases; todos os writers atuais de ingest, proxy review, compare e export final cercam o `UPDATE` com os estados de origem permitidos e fazem rollback por conflito quando o fence não encontra a origem esperada. `PublicOperation` preserva attempt/progresso em `running → waiting → running`. O comando público `apollo.artifacts.lifecycle.transition` aplica lifecycle de artifact com revisão anti-ABA, idempotência persistida, motivo obrigatório e audit trail; `deleted` é tombstone terminal e não apaga bytes. Persistência/lease de waiting, writers futuros, PostgreSQL E2E, deploy e aceite permanecem abertos. Evidência: T-FR-236 e ADR-133.
...
- [ ] Testar sucesso, espera, retry, cancel, falha parcial, stale e conclusão. Parcial F0.029: além das coberturas anteriores, ProjectVersion prova current/superseded, preview histórico disponível/ausente e todas as responses V2 conhecidas que carregam versão, incluindo restore sem alterar accept/reopen, com imutabilidade e adulteração de contrato. Artifact prova matriz exaustiva, quarantine/restore/delete, no-op convergente, replay, payload divergente, revisão obsoleta, ressurreição rejeitada e bloqueio imediato da leitura de bytes fora de `available`. PostgreSQL E2E, deploy e aceite permanecem abertos. Evidência: T-FR-236 e ADR-133.
