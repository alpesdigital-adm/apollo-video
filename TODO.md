...
- [x] Criar `WorkspaceMember` a partir de identidade autenticada e papel ativo. Evidência F0.031: o run hospedado `30768936087` aplicou `V2HumanIdentity` (`issuer + subjectHash`) e `V2WorkspaceMember` em PostgreSQL limpo; sign-in autenticado provisiona uma membership ativa com papel explícito, replay preserva ID/papel sem elevação, cada sessão referencia member/workspace por FK e a autenticação propaga `delegatedUserId`/papel. O teste com duas instâncias comprovou que suspender identity/member nega a sessão. O run passou 810 testes, schema de 135 tabelas/666 índices/514 FKs, build, integrações, goldens e Compose com cleanup. A identidade desta etapa é o bootstrap isolado; substituição por OIDC conforme ADR-142 continua aberta.
