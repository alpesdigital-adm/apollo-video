...
- [ ] Vincular client, workspace e delegated user ao audit context. Parcial local F0.036: Bearer e sessão humana constroem o mesmo `ExternalAuditContext` congelado e todo `requireScope` revalida client, credential, workspace, environment e delegação antes de autorizar. Comandos de containment e de administração de clients/credentials persistem internamente o tuple completo e seu hash canônico, sem ampliar responses públicas. Create/rotate incluem o hash no fingerprint para impedir replay por outra credential/member atrás do mesmo client; revoke e seu command imutável agora são uma transação serializável e concorrência converge. Migrations removem commands, replays e revogações pré-contrato impossíveis de atribuir honestamente. PostgreSQL/HTTP hospedado, ledger exaustivo para as demais capabilities, deploy e aceite permanecem abertos. Evidência: T-FR-242 e ADR-134.
