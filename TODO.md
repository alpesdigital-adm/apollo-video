...
- [ ] Vincular client, workspace e delegated user ao audit context. Parcial local F0.036: Bearer e sessão humana constroem o mesmo `ExternalAuditContext` congelado e todo `requireScope` revalida client, credential, workspace, environment e delegação antes de autorizar. Comandos de containment, administração de clients/credentials e agora create/status de endpoint/subscription de webhook persistem internamente o tuple completo e seu hash canônico, sem ampliar responses públicas. As quatro mutações de webhook exigem o ator no Application service, incorporam seu hash ao fingerprint e gravam intenção+efeito na mesma transação; replay concorrente só converge quando command, base revision, target status e actor coincidem. PostgreSQL/HTTP hospedado, signing-secret/replay e as demais famílias do ledger, deploy e aceite permanecem abertos. Evidência: T-FR-242, T-FR-244 e ADR-134.
...
- [ ] Criar administração de webhooks, subscriptions e delivery diagnostics. Evidência local F0-038: a API externa cobre endpoint create/list/read/status/challenge, subscription create/list/read/status, signing-secret lifecycle, delivery list/read/replay e paginação diagnóstica; a UI de configurações consome exclusivamente essas rotas, apresenta timeline de attempts, lifecycle reversível, subscriptions, rotação e replay. Create/status de endpoint/subscription agora também persistem command administrativo atômico e actor-bound; o E2E HTTP/PostgreSQL foi preparado para exigir exatamente uma linha por concorrência/replay. Signing-secret lifecycle e replay ainda não aderiram a esse ledger. O gate estrutural fixa autenticação, catálogo e paridade das 15 ações. E2E real em browser/PostgreSQL, operações terminais, deploy e aceite ainda impedem fechar a caixa.
