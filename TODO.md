...
- [ ] Definir estados válidos de projeto, versão, job, item batch e artifact. Parcial F0.029: `Project` agora possui enum fechado para as 14 fases e uma projeção visível canônica, além das projeções já fechadas de `PublicOperation`, `ProductionBatch`, itens, invalidation version-scoped e artifact. Create/list e as duas leituras de workspace usam a projeção; estado de versão permanece aberto. Evidência: T-FR-236 e ADR-133.
...
- [ ] Mapear estado técnico para label, progresso e ação na UI. Parcial F0.029: operações v7, batch v2, invalidations v2, `apollo.artifacts.read` v3, `apollo.projects.list` v2, `apollo.projects.create` v3 e `project-workspace` v6 retornam projeções visíveis aditivas; versões anteriores permanecem publicadas. Evidência: T-FR-236 e ADR-133.
- [ ] Testar sucesso, espera, retry, cancel, falha parcial, stale e conclusão. Parcial F0.029: operações cobrem sete estados; batch cobre falha parcial; invalidation cobre stale relacional; artifact cobre seu lifecycle; Project cobre as 14 fases, projeção pública, matriz exaustiva, convergência, terminais, saltos inválidos e fence persistido de ingest. PostgreSQL E2E dos demais writers, estado/transições de versão e artifact, deploy e aceite permanecem abertos. Evidência: T-FR-236 e ADR-133.
