...
- [ ] Definir estados válidos de projeto, versão, job, item batch e artifact. Parcial F0.029: `PublicOperation`, `ProductionBatch`, itens, invalidation version-scoped e artifact possuem enums/projeções visíveis fechados. Artifact distingue `available|quarantined|deleted` de `stale-output`; estado agregado de projeto ainda precisa convergir. Evidência: T-FR-236 e ADR-133.
- [ ] Implementar transições server-side e rejeitar saltos inválidos. Parcial F0.029: `PublicOperation` ganhou transições puras `running → waiting → running`, preservando attempt/progresso e recusando espera fora de execução ou retomada para fase anterior. Artifact agora falha fechado no adapter e presenter para estado desconhecido, mas persistência/lease de waiting e transições explícitas de projeto/artifact permanecem abertas. Evidência: T-FR-236 e ADR-133.
- [ ] Mapear estado técnico para label, progresso e ação na UI. Parcial F0.029: operações v7, batch v2, invalidations v2 e `apollo.artifacts.read` v3 retornam projeções visíveis aditivas; versões anteriores permanecem publicadas. Projeto agregado ainda não usa a projeção. Evidência: T-FR-236 e ADR-133.
- [ ] Testar sucesso, espera, retry, cancel, falha parcial, stale e conclusão. Parcial F0.029: operações cobrem sete estados; batch cobre falha parcial; invalidation cobre stale relacional; artifact cobre `available`, `quarantined`, `deleted`, imutabilidade e estado inventado fail-closed. Projeto agregado permanece aberto. Evidência: T-FR-236 e ADR-133.
