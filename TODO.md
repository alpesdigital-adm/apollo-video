...
- [ ] Enfileirar proxy/range render mínimo e manter outputs válidos. Parcial F0-027: `manual-edits.apply/v1` continua enfileirando o worker V2 existente, que deriva do Command persistido um único range stale, recompõe apenas esse trecho e monta um MP4 completo reutilizando prefixo/sufixo do proxy concluído da versão-base, com validação de hash, tamanho, duração e lineage. Sem proxy reutilizável ou quando todo o timeline é afetado, executa render V2 completo. Ainda faltam seleção sem render, múltiplos ranges/rates, outros Command types, E2E PostgreSQL, deploy e aceite. Evidência: T-FR-233 e ADR-133.
