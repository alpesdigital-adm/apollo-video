...
- [ ] Definir interfaces entre Provider Registry, Director, Compiler, Renderer e Critics. Evidência parcial integrada F0.032: ports existentes e o fluxo estrutural fixam a direção das dependências, mas a interface canônica completa entre os cinco componentes ainda não foi consolidada nem provada em E2E. Evidência: T-F0.032 e ADR-134.
...
- [ ] Criar architecture tests e um fluxo fake atravessando todos os componentes. Evidência parcial integrada F0.032: T-F0.032 testa a matriz fail-closed, acesso Web proibido e um fluxo executável `web → application → domain → orchestrator → provider → critic → compiler → renderer`; ainda é uma prova estrutural sintética, não um E2E integral do produto. Evidência: T-F0.032 e ADR-134.
