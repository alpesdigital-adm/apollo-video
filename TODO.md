...
- [ ] Configurar audit, typecheck, unit, migration check, contract gate, build e integrações Postgres/API no CI. Parcial F0.030: o workflow contém esses gates, usa a mesma imagem PostgreSQL 16 + pgvector exigida pelas migrations e bloqueia drift dos contratos de infraestrutura via `infra:validate`; faltam ampliar as integrações descobertas, golden determinístico e confirmação hospedada do commit corretivo. Evidência: `.github/workflows/ci.yml`.
