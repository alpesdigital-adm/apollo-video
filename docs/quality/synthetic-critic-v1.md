...
## Complemento W24.1 — crítico no worker (2026-09-22)

O worker de provider agora executa o crítico especializado depois da ingestão e antes de `approved`. O contexto vem do job, plano/bloco, generation quando existe, snapshot e head atual do presenter, rights atuais e ledger de artifacts; payload público não fornece veredito nem fatos de aprovação. O relatório é persistido primeiro e seu `reportHash` passa a ser o `criticResultHash` do job. `evaluationContextHash` sela projeto/bloco, provenance do adapter, bytes alvo e áudio, alignment, script, profile, expectativa e policy. Replay exige esse contexto inteiro; os mesmos bytes com um alignment novo produzem outra opinião, enquanto repetição do contexto exato converge. Consumers abrem o `reportHash` exato selado no job em vez de escolher o relatório mais recente do artifact. A migration `20260922153000_synthetic_critic_evaluation_context` substitui a unicidade estreita de bloco/artifact/policy pela identidade completa; a coluna permanece nullable somente para que relatórios históricos sejam legíveis. Histórico sem `expectationHash` ou `evaluationContextHash` não autoriza cache, settle, compilação, master ou produção sob policy corrente.

TTS livre usa `durationMode=alignment`: compara o fim do alignment persistido e validado com a duração contada dos samples PCM mono 16 kHz decodificados. Isso evita padding de contêiner MP3 sem usar o próprio áudio como meta, sem ampliar o limite de 34 ms e sem alegar calibração live. Avatar preserva duração fixa do range aprovado e permanece fail-closed quando não existe alignment materializado do range ou evidência independente do output para identidade.

A etapa longa de ffprobe/ffmpeg renova lease com CAS; perda de lease aborta o subprocesso, aguarda sua saída e impede aprovação. A jornada PostgreSQL controlada `synthetic-production-journey.e2e.mjs` removeu a chamada manual ao serviço: 13 jobs produziram 13 reports pelo worker, silêncio foi rejeitado, retry recuperou o bloco e três reusos não geraram nova chamada. Essa é evidência técnica controlada local; não prova provider live, deploy ou aceite de F3.019.
