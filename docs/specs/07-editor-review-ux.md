...
11. Leituras do editor são coordenadas, não disparadas por identidade instável de objetos: um GET idêntico em voo é compartilhado; a chave inclui método, URL e escopo; uma resposta ou falha que chega depois de o projeto, sessão ou geração da leitura mudarem é descartada; e uma mutação invalida a geração afetada antes de buscar estado novo, sem apagar autenticação fechada nem a janela de `Retry-After`. O polling de operação aguarda a própria rodada, para em estado terminal e pausa com a aba oculta. 401 encerra inclusive requests já em voo; 403 e 409 permanecem recusados para leituras automáticas até retry explícito ou mudança real de escopo; 429 preserva código, request id e mensagem estruturada enquanto recalcula o tempo restante, e trocar A→B→A não burla seu prazo. Falha ou loading de review/proxy review mantém o preview já autorizado inspecionável, mas limpa aprovação stale, bloqueia na renderização e também nos handlers as ações dependentes, inclusive export final. Conflito de persistência recebe mensagem genérica honesta, sem atribuir toda ocorrência à auditoria histórica. (Wave 22, 2026-09-12: unitários focados e E2E local `b6dd0001a1e4` aprovados; 401/429 no browser são transporte controlado; CI final/VPS, deploy e aceite pendentes.)
12. O relatório de paridade UI/API deve falhar quando uma chamada canônica `reads.read(...)` não contém URL estática classificável no object literal. Descritores indiretos não podem remover capabilities silenciosamente do relatório; receptores `.read` de outras bibliotecas não são classificados como transporte Apollo. O teste de falsificação preserva explicitamente annotations e proxy reviews. (Wave 22, 2026-09-12: `scripts/generate-ui-capability-parity-report.mjs`, `tests/v2/capability-registry.test.mjs`; regeneração/validação do artefato publicado e gates hospedados continuam separados.)
13. Uma mudança de LUT no projeto pertence à nova ProjectVersion: `plan.projectVersionId` aponta para a versão-resultante e o snapshot imutável é criado atomicamente com Command, ProjectVersion e avanço do head. CAS e replay revalidam a versão-base e a seleção resolvida sem alterar o snapshot-base. Proxy/final reservam identidade lógica própria, independente do hash dos bytes, de modo que outputs byte-idênticos de projetos diferentes não compartilham artifact, manifest ou lineage. (PR53, 2026-09-12: prova local 2.280/2.280 testes unitários sem skip, build/ESLint, PostgreSQL controlado com dois projetos byte-idênticos e timeline pós-LUT 200, e E2E local do editor aprovado; CI final/VPS, deploy e aceite pendentes.)
