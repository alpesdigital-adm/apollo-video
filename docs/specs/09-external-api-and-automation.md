...
Administração de endpoint e subscription de webhook segue o mesmo binding. As operações create e status-set aceitam `AuthenticatedExternalActor`, materializam internamente `WebhookAdministrationCommand` com target, `targetStatus` quando aplicável, base revision, credential/delegação e context hash, e persistem essa linha na mesma transação do efeito. Create inclui o hash no fingerprint idempotente; status só é considerado replay se o command anterior tiver o mesmo actor, target, base e intenção. O contrato ainda não abrange provision/rotate/activate/cancel de signing secret, challenge ou replay administrativo de delivery/event.
