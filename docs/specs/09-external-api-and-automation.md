...
Toda criação externa de trabalho durável deve preservar esse mesmo vínculo. Materialização, render, ingest, proxy, export, Diretor, source cleanup e long-form recebem `AuthenticatedExternalActor`, reaplicam o scope no Application boundary, incorporam `actorContextHash` ao fingerprint e persistem credential, environment, authentication kind e delegação na `PublicOperation` ou no aggregate específico. Quando há aggregate e operação, ambos carregam o mesmo hash e a hidratação exige igualdade. Linhas anteriores ao contrato permanecem fisicamente identificáveis pelo grupo nulo, mas são ilegíveis para a aplicação até o reset pré-produção; é proibido reconstruir autoria por backfill.
...
Mutações de produção em lote seguem a mesma fronteira externa. Batch create/action/item action, partial retry, alignment/review, take library/selection, compatibility graph, variant recipe, portfolio preflight e batch edit preflight/commit recebem o actor autenticado completo e reaplicam `projects:write` no Application service compartilhado. Fingerprints, replay, recuperação concorrente e o par preflight/commit são vinculados ao context hash. Os aggregates e commands imutáveis persistem o tuple completo e o adapter recalcula sua identidade na hidratação; linhas pré-contrato ou adulteradas falham fechado em vez de receber autoria inferida.

Mutações públicas de inteligência do projeto formam uma família auditável fechada: catálogos de fala/evidência/validação, documentos/avaliações/reuso/escala semânticos, desconstrução de source e relatório de contaminação. As nove rotas entregam o `AuthenticatedExternalActor` completo, os Application services reaplicam `projects:write`, e seis repositories vinculam fingerprint, replay e recuperação concorrente ao context hash. Nove aggregate heads persistem o tuple e o recalculam na hidratação. A migration não atribui identidades inferidas a linhas históricas; o grupo é all-null ou completo no banco, mas a aplicação recusa a forma nula.

Commands editoriais externos formam outra família fechada sobre o aggregate compartilhado `V2EditCommand`. Remoção de fala, replacement de transcript, edição manual/restore, aplicação de review patch individual/em lote e accept/reopen de comparação recebem `AuthenticatedExternalActor`, reaplicam `projects:write` no Application boundary e vinculam todo replay ao context hash. Seis repositories persistem e reidratam credential, environment, authentication kind e delegação; uma credential diferente do mesmo client não herda idempotência. A constraint PostgreSQL usa `IS NOT NULL` explícito para os campos externos obrigatórios, evitando que `UNKNOWN` aceite tuple parcial. Nenhum backfill inventa autoria, e Commands internos produzidos pelo worker do Diretor continuam separados até que seu provenance durável seja modelado.

Mutações de avaliação do projeto formam uma família externa maior, mas ainda fechada: color pipeline, extração contígua, proof need/integrity/mode, acknowledgement de proxy review, quality iteration, asset selection, MVP core gate e criação/decisão de validation envelope. Onze rotas entregam o ator autenticado completo a dez fronteiras Application/Prisma. `projects:write`, workspace e context hash são revalidados antes de efeito ou replay; a credential e eventual delegação são persistidas e recalculadas na hidratação. Os onze aggregates usam tuple all-null ou completo somente para permitir reset seguro sem autoria inventada; o runtime recusa a forma nula. Diretor, long-form e processamento hierárquico continuam separados porque compartilham produção durável por workers.

Produção direta e produção durável de análise usam a mesma identidade iniciadora, mas proveniências diferentes. Em `external-request`, Diretor, long-form moments e hierarchical processing exigem `AuthenticatedExternalActor`, `projects:write` e ausência de lineage de worker. Em `long-form-stage`, o Application service recusa actor transitório e exige o `ApiAccessAuditContext` da `PublicOperation`, mais operation/workflow/stage/input hash/idempotency key idênticos ao fence. O Command ou run persistido guarda os dois e o adapter recalcula o tuple, compara a operação e o checkpoint e rejeita replay cross-credential. Worker, lease owner e process ID nunca são promovidos a autor do produto.

Diarização e análises contíguas derivadas também são exclusivamente `long-form-stage`. O worker propaga o audit context já persistido; os três run heads (`SpeakerDiarizationRun`, `ContiguousEvidenceRun` e `ContiguousEvaluationRun`) guardam credential/delegação e operation/workflow/stage/checkpoint completos. Os produtores contíguos não possuem método de persistência sem fence. Antes de executar provider, o serviço valida workspace, project e stage; no commit, o adapter exige operação `running` com o mesmo context hash e checkpoint ativo. Replay com outra credential, tuple parcial, provenance divergente ou lease perdido falha fechado.
...
Importação de LUT, criação de sua próxima versão, alteração de status e definição do padrão do workspace recebem o mesmo `AuthenticatedExternalActor` sob `projects:write`. `WorkspaceLutVersion`, `WorkspaceLutStatusCommand` e `WorkspaceLutDefaultVersion` guardam o tuple completo e o hash canônico; idempotência inclui esse hash e a hidratação o recalcula. A projeção pública continua redigida. Registros pré-contrato são removidos no reset autorizado, pois client ID isolado não prova a credential nem a eventual pessoa delegada.

`set-project-lut-selection` não cria outra representação de comando. O mesmo `V2EditCommand` recebe o audit tuple quando a origem é externa, enquanto `director|system` permanecem origens internas explícitas sem campos de autenticação. A entrada externa exige `projects:write`; o context hash substitui a projeção reduzida no fingerprint. Um api-client sem tuple, com tuple adulterado ou vindo de outra credential falha antes de leitura/replay. As colunas são opcionais como grupo porque o aggregate também representa trabalho interno, mas, quando presentes, o banco exige a forma completa e coerente.
...
Para todo tipo executável, a sequência de fases é também a fonte canônica do progresso persistido. `queued` usa `0/total`; `running` exige que `completed` seja o índice exato de `phase`; `succeeded` exige `completed === total`; espera, retry, falha e cancelamento preservam um checkpoint entre zero e a última fase executável. `total` e `unit` são obrigatórios e derivados do tipo. Avanço por cache hit pode saltar trabalho desnecessário, mas o novo estágio e seu contador continuam inseparáveis. Domínio, hidratação Prisma e `public_operations_progress_check` aplicam a mesma matriz; nenhuma camada aceita percentual fornecido pelo cliente.
...
Cancelamento e retry são comandos auditáveis, não apenas updates de status. Cada transição efetiva persiste um `PublicOperationControlCommand` imutável com ação, status anterior/resultante, instante e audit tuple completo na mesma transação da `PublicOperation` e dos eventos de outbox. Uma chamada convergente que encontra o resultado já aplicado retorna o estado atual sem criar command; corrida concorrente produz exatamente um command para o vencedor. Retry de uma operação cancelada antes da primeira tentativa pode voltar a `queued`; após tentativa iniciada, ou vindo de `failed`, volta a `retrying`.
...
Writers adicionais seguem a mesma regra de commit: annotation criada/aplicada produz `annotation.created`/`annotation.resolved`; lifecycle realmente alterado para `available`/`quarantined` produz `artifact.ready`/`artifact.rejected`; ação `suspend` que altera um API client produz `client.suspended`. No-op, replay e rollback produzem zero linhas. Texto, screenshot, reason administrativo, credenciais e detalhes de storage são proibidos nesses payloads. `budget.threshold.reached` usa a política `production-batch-budget-thresholds/v1`: o ledger canônico é a soma dos custos efetivamente persistidos nos steps, com eventos em 80% e 100% do teto. Reserva, preflight ou estimativa isolada não autorizam evento; cada cruzamento é persistido atomicamente com a ação de batch que o causou.
