...
> **Complemento W24.1.** A aprovação sintética é produzida dentro do worker, sob lease renovável e antes do estado terminal. TTS público referencia plano e bloco persistidos; o servidor deriva texto/hash/profile/use/market/locale e revalida head e rights atuais antes do submit e no critic. Para fala livre, temporal-integrity v2 mede samples PCM decodificados contra o fim do alignment durável e monotônico. Consumers exigem `expectationHash`, `evaluationContextHash`, a versão de thresholds vigente e o `reportHash` exato selado no job; mesmos bytes sob outro alignment são outra avaliação, não replay. A migration `20260922153000_synthetic_critic_evaluation_context` preserva a leitura histórica sem transformar registros sem contexto verificável em aprovação. O avatar só pode aprovar quando a evidência do range realmente submetido e evidência independente do output para identidade existirem; master inteiro não é apresentado como áudio recortado. Esses gates não transformam detector controlado em prova perceptual ou provider live.
