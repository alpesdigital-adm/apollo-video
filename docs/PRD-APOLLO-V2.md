...
Senha humana nunca é uma credencial de integração e não deve ser enviada a MCP, Director ou provider. O ADR-142 seleciona OIDC Authorization Code + PKCE para produção, sessão opaca server-side com idle de 30 minutos, expiração absoluta de 12 horas, rotação/revogação duráveis e recuperação de credencial exclusivamente pelo IdP. O bootstrap scrypt local permanece apenas para desenvolvimento isolado e bloqueia produção até a substituição. A etapa transitória já persiste sessões revogáveis, idle timeout, throttle distribuído, audit redigido, identidade humana e `WorkspaceMember` ativo no PostgreSQL; as páginas V2 exigem essa sessão no boundary SSR. Implementação OIDC e identificador opaco rotacionável continuam requisitos abertos.
