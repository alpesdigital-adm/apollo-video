...
Incremento local F0.102: o runtime de embedding semântico agora é selecionado pelo environment autenticado. Sandbox ignora deliberadamente qualquer chave/configuração externa e usa fake determinístico; cada chamada persiste receipt content-addressed com hashes, unidades, custo simulado e prova de zero chamadas externas, consultável por capability administrativa paginada. Production não aceita deterministic/fake e falha fechado sem OpenAI configurada. O E2E PostgreSQL/HTTP foi preparado com chave externa sentinela, mas não executado; outros providers, deploy e aceite ainda faltam.
