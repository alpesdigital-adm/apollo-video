...
As quatro mutações de LUT do workspace seguem o mesmo contrato. Importação, nova versão, lifecycle e padrão exigem `projects:write`, vinculam o fingerprint ao context hash e persistem o tuple autenticado completo na versão ou command imutável. A hidratação recalcula a identidade antes de aceitar leitura ou replay; outra credential do mesmo client produz mismatch. Como os registros anteriores continham somente o client técnico, o reset pré-produção elimina esse conjunto em vez de fabricar credential ou pessoa delegada. A jornada HTTP/PostgreSQL está preparada para conferir a autoria sem expô-la na response; execução hospedada, seleção de LUT por projeto, deploy e aceite permanecem pendentes.
