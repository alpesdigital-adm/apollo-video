...
Revisão colaborativa também preserva autoria confiável na criação, antes da aplicação editorial. Annotation, proposta individual e lote recebem o ator autenticado completo, reaplicam `projects:write` e tornam idempotência e recuperação concorrente específicas da credential ou sessão delegada. O autor público é uma projeção vinculada por constraint ao ator: Bearer aparece como `api-client`; sessão humana, como o usuário delegado. O tuple privado não atravessa os presenters públicos. Persistência e hidratação recusam grupo parcial, hash adulterado, troca de credential e autoria pública divergente. A prova atual é local, com E2Es PostgreSQL preparados mas ainda não executados; deploy e aceite permanecem pendentes.

Análises derivadas do workflow long-form seguem a mesma regra sem converter o worker em autor. O run de diarização, os cinco runs de evidência contígua e o run de avaliação carregam o audit context iniciador e a proveniência `long-form-stage` completa, vinculada à operação, workflow, estágio, input hash e chave do checkpoint. Não existe caminho de persistência contígua fora do lease; operação corrente, fingerprint, replay, FKs e fence precisam concordar com a mesma credential/delegação. Tuples históricos nulos não recebem backfill e falham na hidratação. A prova local inclui serviços, adapters e worker; a jornada PostgreSQL foi preparada, mas execução isolada, deploy e aceite permanecem pendentes.
...
Incremento local F0.100–F0.101/F0.103: o gateway autenticado registra admission transacional por capability antes do handler, avaliando separadamente budget e sinais de anomalia do workspace e do client. Rate, concurrency/reservas, quota e spend continuam hard limits; uma policy versionada/content-addressed mede picos de requests e spend contra baseline e error-rate sobre operações terminais. Admissions/alerts v2 preservam policy, janela, observado/threshold e eventual recovery humano. O audit v3 normaliza admissions v1/v2 e a capability pública `/v1/governance/alerts` pagina alertas redigidos. Recovery é permitido somente a sessão humana administradora allowlisted e não contorna hard limits. O PostgreSQL E2E está preparado, mas não foi executado; custo real reconciliado, sinais específicos de mutation/generation/download, demais providers sandbox, deploy e aceite permanecem pendentes.

Incremento local seguinte F0.100: o lifecycle das policies passou a ser operável pelas capabilities públicas list/set/delete, com `clients:admin`, workspace binding, confirmação, CAS, idempotência actor-bound e command durável content-addressed na mesma transação da policy. Client policy só aceita client do mesmo workspace/environment. Operações gratuitas continuam sujeitas a rate, mas não são bloqueadas por dimensões de concurrency/quota/spend que não consomem, preservando recuperação. O E2E PostgreSQL/HTTP foi atualizado para atravessar essas rotas, porém não foi executado neste host; custo real, sinais específicos por operação, demais providers sandbox, deploy e aceite permanecem pendentes.

Incremento local F0.102: runtimes de embedding semântico, transcrição do ingest, transcript long-form e diarização agora são selecionados pelo environment autenticado persistido na operação. Sandbox ignora deliberadamente qualquer chave/configuração externa e usa fakes determinísticos; cada chamada persiste receipt content-addressed v1/v2 com hashes, unidades, custo simulado e prova de zero chamadas externas, consultável pela capability administrativa paginada v2. Production não aceita deterministic/fake, falha fechado sem OpenAI/Groq configurados e rejeita provider/model/version caller-selected antes de enfileirar long-form. O E2E PostgreSQL/HTTP foi preparado, mas não executado; providers sintéticos, deploy e aceite ainda faltam.
