...
The Next.js/React supported-line upgrade is also closed. Official support documentation checked on 2 August 2026 identifies Next.js 16.x as Active LTS and React 19.2 as the current documented line, including the published 19.2.7 patch used by both the app and renderer. Commit `c350181` performed the controlled upgrade and async request API codemod; the current baseline is Next 16.2.12. Hosted run `30777268776` regenerated App Router types, typechecked the asynchronous dynamic params, validated all 189 public capabilities without baseline drift, bundled and smoke-rendered Remotion with real Chromium, built the production application and passed the complete PostgreSQL, MinIO, FFmpeg and supervised Compose matrix. This closes compatibility of the upgrade, not the remaining F0.033 model/provider/storage work or production acceptance of those separate capabilities.
