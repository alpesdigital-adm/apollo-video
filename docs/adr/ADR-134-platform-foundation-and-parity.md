...
Hosted run `30778629569` closes the named non-major dependency advisories with both a live registry audit and a deterministic lockfile gate. The machine-readable baseline and `platform:validate` require exact `uuid` 11.1.1, exact `postcss` 8.5.18 in both root and Remotion lockfiles, the root PostCSS override and complete absence of `form-data` from both dependency trees. Root and renderer audits reported zero vulnerabilities after clean installs, and the complete hosted matrix passed. Future dependency changes therefore fail CI if they silently reintroduce one of these resolved conditions, independently of registry advisory availability.
