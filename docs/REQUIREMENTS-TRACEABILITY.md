...
| FR-134 | Contiguous extraction | S1,S3 | D1,D2 | janela autocontida duração-alvo | T-FR-134 domínio 4 casos + application 3 casos: contrato `contiguous-extraction/v1` seleciona deterministicamente uma única faixa semântica autorizada, pontua autocontenção/densidade/integridade/áudio/visual somente com evidence refs, compila StoryPlan e EditPlan de um clip sem síntese multi-range ou zoom automático e vincula hash à lineage. O application service recebe apenas objetivo/tópico/duração, lê candidates e scores pela porta confiável, persiste uma extração e converge idempotência sem reavaliar. Schema/migration V2 para avaliações imutáveis de moment e extrações imutáveis foi aplicado do zero em PostgreSQL/pgvector local (110 tabelas/565 índices/434 FKs), mas o comportamento permanece implementado isoladamente; adapter, produtor das avaliações, API `/v1`, golden E2E de 2h, UI, deploy e aceite estão pendentes |
