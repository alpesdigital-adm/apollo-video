...
| FR-133 | Long-form indexing | S3 | D1,D2,D4 | workflow probe→transcript→diarization→chunks→moments com checkpoints, referência tipada ao resultado persistido, parcial pesquisável, budget e retomada sem duplicação | T-FR-133 state machine 6 casos + application/API contract 4 casos + worker orchestration 4 casos + transcript 3 casos + diarização 9 casos focados e 1 integração FFmpeg local + derived stages 6 casos e fencing Prisma controlado 3 casos; schema/migration/repositório/API pública e orquestração durável com lease fencing estão implementados localmente. Transcript ausente agora prepara áudio do artifact imutável, reutiliza saída exata sem novo custo, valida provider/budget/rights/lease e persiste `V2MediaTranscript` antes de publicar a referência; diarização persiste clusters anônimos; chunks/moments preservam lineage e derivam `speakerIds` somente por sobreposição temporal. Worker runtime integral, PostgreSQL E2E do workflow completo, execução real controlada dos providers, deploy e aceite permanecem pendentes |
