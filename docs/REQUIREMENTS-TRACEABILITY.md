...
| FR-133 | Long-form indexing | S3 | D1,D2,D4 | workflow probe→transcript→diarization→chunks→moments com checkpoints, referência tipada ao resultado persistido, parcial pesquisável, budget e retomada sem duplicação | T-FR-133 state machine 6 casos + application/API contract 4 casos + worker orchestration 4 casos + diarização 9 casos focados e 1 integração FFmpeg local + derived stages 5 casos e fencing Prisma controlado 2 casos; schema/migration/repositório/API pública e orquestração durável com lease fencing estão implementados localmente; diarização prepara input imutável, valida rights/lease/budget e persiste clusters anônimos; chunks/moments publicam sob o mesmo lease, preservam lineage e derivam `speakerIds` somente por sobreposição temporal. Stage de transcript para fontes ainda não transcritas, worker runtime integral, PostgreSQL E2E, execução real controlada do provider, deploy e aceite permanecem pendentes |
