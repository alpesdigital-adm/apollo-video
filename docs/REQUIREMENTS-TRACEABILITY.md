...
Atualização local de FR-180 — o contrato isolado `color-plan/v1` agora materializa color space, transfer, primaries, matrix, range e bit depth na entrada, na saída e em cada estágio `technical → match → creative-lut → output`. Todos os quatro estágios precisam estar explícitos; a resolução ordena o pipeline independentemente da ordem de declaração, permite override source/camera/segment somente por substituição, recusa estágio duplicado dentro do mesmo layer, exige continuidade colorimétrica entre estágios, vincula provider/version/parâmetros e seu SHA-256 e só permite LUT em `creative-lut` ativa com artifact ID/hash imutáveis. O resultado `resolved-color-pipeline/v1` possui manifesto e hash canônico. Isso é implementação de domínio isolada: persistência, application service/API, adapter FFmpeg, fixtures SDR visuais reais, integração ao RenderInput/renderer, E2E, deploy e aceite continuam pendentes; F2.025 permanece aberto.
