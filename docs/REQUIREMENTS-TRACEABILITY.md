...
| FR-133 | Long-form indexing | S3 | D1,D2,D4 | workflow probe→transcript→diarization→chunks→moments com checkpoints, referência tipada ao resultado persistido, parcial pesquisável, budget e retomada sem duplicação | T-FR-133 state machine 6 casos + application/API contract 4 casos + worker orchestration 4 casos + diarização 9 casos focados e 1 integração FFmpeg local; schema/migration/repositório/API pública e orquestração durável com lease fencing implementados localmente; o stage de diarização prepara input imutável, valida rights/lease/budget, usa o contrato oficial `gpt-4o-transcribe-diarize`, persiste clusters anônimos e pode ser composto por ambiente, sem chamada externa nos testes; adapters com lease para chunks/moments, PostgreSQL E2E, execução real controlada do provider, deploy e aceite permanecem pendentes |
