...
| FR-133 | Long-form indexing | S3 | D1,D2,D4 | workflow probe→transcript→diarization→chunks→moments com checkpoints, parcial pesquisável, budget e retomada sem duplicação | T-FR-133 state machine 6 casos + application/API contract 4 casos; schema/migration/repositório/API pública implementados localmente; worker, PostgreSQL E2E, deploy e aceite pendentes |
