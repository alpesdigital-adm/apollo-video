...
Canonical source types live in the per-aggregate modules under `src/v2/domain/` — for capture, `capture-session.ts`, `session-time.ts`, `piecewise-clock-map.ts`, `track-coverage.ts` and `sync-diagnostic.ts`; for media, `media-segment.ts` and `media-artifact.ts`. `src/v2/domain/canonical-types.ts` is a 2026-07-17 sketch that no file in `src/`, `tests/` or `scripts/` imports, and is not the source of truth. OpenAPI, JSON Schemas and MCP tools are generated from the Public Capability registry and validated by `api:v1:validate`.
